import{c as Mt,i as y,r as rt,d as ot,g as it,u as bt}from"./index-DklScFds.js";const ut="https://libraryratingsdb.zxcsixx.workers.dev/api/ratings",qt=5*60*1e3,mt={data:{},lastUpdated:{},get:function(t){const e=this.data[t],s=this.lastUpdated[t];return e&&s&&Date.now()-s<qt?Promise.resolve(e):null},set:function(t,e){return this.data[t]=e,this.lastUpdated[t]=Date.now(),e},batchGet:function(t){const e={cached:{},uncached:[]};return t.forEach(s=>{const a=this.get(s);a?e.cached[s]=a:e.uncached.push(s)}),e},batchSet:function(t){Object.entries(t).forEach(([e,s])=>{this.set(e,s)})}};function Pt(t){const e=new Date(t),a=Math.floor((new Date-e)/1e3);let n=Math.floor(a/31536e3);return n>=1?n+" year"+(n===1?"":"s")+" ago":(n=Math.floor(a/2592e3),n>=1?n+" month"+(n===1?"":"s")+" ago":(n=Math.floor(a/86400),n>=1?n+" day"+(n===1?"":"s")+" ago":(n=Math.floor(a/3600),n>=1?n+" hour"+(n===1?"":"s")+" ago":(n=Math.floor(a/60),n>=1?n+" minute"+(n===1?"":"s")+" ago":"just now"))))}function W(t){return String(t).replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Bt(t){let e=localStorage.getItem("hydra_rating_hash");return e||(e=Math.random().toString(36).slice(2)+Date.now(),localStorage.setItem("hydra_rating_hash",e)),e}function Rt(t){document.querySelectorAll(".rating-modal").forEach(d=>d.remove());const e=document.createElement("div");e.className="rating-modal fixed inset-0 z-[9999] flex items-center justify-center p-2 sm:p-4",e.innerHTML=`
  ...
  `,document.head.appendChild(s);const a=()=>{window.turnstile?window.turnstile.render(".cf-turnstile",{sitekey:"0x4AAAAAABgPUEsL6w8fjG-Z",callback:window.onTurnstileSuccess,"expired-callback":window.onTurnstileExpired,"error-callback":window.onTurnstileError,theme:"dark"}):setTimeout(a,100)};a(),e.querySelector(".close-rating-modal").onclick=l,e.querySelector(".fixed.inset-0").onclick=d=>{d.target===e.querySelector(".fixed.inset-0")&&l()};async function n(d,c="en"){return new Promise((p,g)=>{const h=JSON.stringify({translate:"rapidapi"}),x=new XMLHttpRequest;x.withCredentials=!0,x.addEventListener("readystatechange",function(){if(this.readyState===this.DONE)try{const b=JSON.parse(this.responseText);if(b&&b.translation)p(b.translation);else throw b&&b.status!==200?new Error(b.business_message||"Translation service error"):new Error("Invalid response from translation service")}catch(b){console.error("Translation error:",b,this.responseText),g(b)}}),x.addEventListener("error",()=>{g(new Error("Network error during translation"))});const P=`https://free-google-translator.p.rapidapi.com/external-api/free-google-translator?from=auto&to=${c}&query=${encodeURIComponent(d)}`;x.open("POST",P),x.setRequestHeader("x-rapidapi-key",""),x.setRequestHeader("x-rapidapi-host","free-google-translator.p.rapidapi.com"),x.setRequestHeader("Content-Type","application/json"),x.send(h)})}async function i(d){const c=d.target.closest(".translate-comment");if(!c||c.disabled)return;d.preventDefault(),d.stopPropagation();const p=c.closest(".border-b"),g=p?p.querySelector(".comment-text p"):null;if(!g){console.error("Could not find comment text element");return}let h=c.getAttribute("data-original");if(c.classList.contains("active")){h&&(g.textContent=h,c.innerHTML='<i class="fas fa-language text-xs"></i><span class="hidden sm:inline">Translate</span>',c.classList.remove("active"),c.title="Translate this review"),c.disabled=!1;return}h=g.textContent.trim(),c.setAttribute("data-original",h),c.innerHTML,c.innerHTML='<i class="fas fa-spinner fa-spin text-xs"></i>',c.title="Translating...",c.disabled=!0;try{const x=await n(h,"en");g.textContent=x,c.innerHTML='<i class="fas fa-undo text-xs"></i><span class="hidden sm:inline">Original</span>',c.classList.add("active"),c.title="Show original text"}catch(x){console.error("Translation failed:",x),g.textContent=h,c.innerHTML='<i class="fas fa-exclamation-triangle text-xs"></i><span class="hidden sm:inline">Error</span>',c.title="Error translating. Click to try again."}finally{c.disabled=!1}}const o=e.querySelector("#rating-comments-list");o&&o.addEventListener("click",i);const r=e.querySelector('textarea[name="comment"]'),u=e.querySelector(".char-count");if(r&&u){const d=()=>{u.textContent=r.value.length};r.addEventListener("input",d),d(),e._cleanupCharCounter=()=>{r.removeEventListener("input",d)}}function l(){e._cleanupCharCounter&&e._cleanupCharCounter(),e.remove(),document.body.style.overflow=""}const f=5*60*1e3;let m=1,w="recent",S=0,E=!1,L=0,T=t.title;function $(){const d=`ratings_${encodeURIComponent(T)}`,c=localStorage.getItem(d);if(!c)return null;try{const{data:p,timestamp:g}=JSON.parse(c);if(Date.now()-g<f)return p}catch(p){console.warn("Error reading cache:",p)}return null}function M(d){try{const c=`ratings_${encodeURIComponent(T)}`,p={data:d,timestamp:Date.now()};localStorage.setItem(c,JSON.stringify(p))}catch(c){console.warn("Error saving to cache:",c)}}async function C(d=1,c=!1){if(E)return;const p=e.querySelector("#rating-comments-list");if(!p)return;p.innerHTML='<div class="flex justify-center items-center py-6"><div class="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-emerald-400"></div></div>';const g=c?null:$();if(g){console.log("Using cached data for:",T),A(g,d),Date.now()-L>f&&B(!0);return}await B()}function A(d,c=1){const p=e.querySelector("#rating-comments-list");if(p){if(p.innerHTML="",d.avg!==void 0){const g=parseFloat(d.avg).toFixed(1),h=parseInt(d.total)||0,x=e.querySelector("#rating-modal-stars"),P=e.querySelector("#rating-modal-avg"),b=e.querySelector("#rating-modal-total");if(x){x.innerHTML="";const F=Math.round(parseFloat(d.avg));x.innerHTML="★".repeat(F)+"☆".repeat(5-F)}P&&(P.textContent=`${g} out of 5`),b&&(b.textContent=`${h} ${h===1?"review":"reviews"}`)}if(!d.comments||d.comments.length===0){p.innerHTML=`
    ...
    `;d.innerHTML=p}window.loadPage=d=>{if(!k||!k.comments)return;const c=Math.ceil(k.comments.length/10);d<1||d>c||(m=d,R())},e._vm={loadPage:window.loadPage};async function B(d=!1){var x,P;if(E)return;E=!0;const c=e.querySelector("#rating-comments-list");!d&&c&&(c.innerHTML='<div class="text-center py-4 text-gray-400">Loading comments...</div>');const p=`comments_${T}`,g=5*60*1e3,h=mt.get(p);if(h&&Date.now()-h.timestamp<g){k=h,R(),E=!1;return}try{const b=await fetch(`${ut}?source=${encodeURIComponent(T)}&all=true`);if(!b.ok)throw new Error(`Failed to fetch comments: ${b.statusText}`);const F=await b.json();k={...F,comments:F.comments||[],pagination:{total:((x=F.comments)==null?void 0:x.length)||0,page:1,totalPages:1,perPage:((P=F.comments)==null?void 0:P.length)||0}},mt.set(p,{...k,timestamp:Date.now()}),L=Date.now(),R()}catch(b){console.error("Error fetching comments:",b),h?(k=h,R()):!d&&c&&(c.innerHTML=`
          <div class="text-red-500 text-center py-4">
            Failed to load comments. Please try again later.
            ${b.message?`<div class="text-xs mt-1">${b.message}</div>`:""}
          </div>
        `)}finally{E=!1}}function R(){if(!k||!k.comments)return;const d=J([...k.comments],w),c=10,p=Math.ceil(d.length/c),g=(m-1)*c,h=d.slice(g,g+c);k.pagination={total:d.length,page:m,totalPages:p,perPage:c},A({...k,comments:h,pagination:k.pagination},m)}let k=null;function J(d,c){if(!d)return[];const p=[...d];switch(c){case"recent":return p.sort((g,h)=>{const x=g.timestamp?new Date(g.timestamp).getTime():0;return(h.timestamp?new Date(h.timestamp).getTime():0)-x});case"high":return p.sort((g,h)=>(h.rating||0)-(g.rating||0));case"low":return p.sort((g,h)=>(g.rating||0)-(h.rating||0));default:return p}}function At(){const d=e.querySelector("#rating-sort-select");d&&d.addEventListener("change",c=>{w=c.target.value,m=1,R()})}At(),C(),C();const V=e.querySelector("#submit-rating-form"),O=e.querySelector("#rating-form-error"),j=V.querySelector('button[type="submit"]');j.disabled=!0,V.onsubmit=async d=>{var c;d.preventDefault(),O.textContent="",O.className="mt-1 text-red-400 text-xs min-h-[20px] bg-red-900/30 rounded py-1.5 px-2 border border-red-900/20";try{const p=new FormData(V),g=(p.get("nickname")||"").trim(),h=p.get("rating"),x=(p.get("comment")||"").trim(),P=(c=document.querySelector('[name="cf-turnstile-response"]'))==null?void 0:c.value;if(!P)throw new Error("Please complete the CAPTCHA verification");if(!g)throw new Error("Please enter a nickname");if(!h||isNaN(h)||h<1||h>5)throw new Error("Please select a valid rating between 1 and 5");if(!x||x.split(/\s+/).length<3)throw new Error("Please enter a message with at least 3 words");j.disabled=!0,j.innerHTML='<i class="fas fa-spinner fa-spin mr-2"></i> Submitting...',O.classList.add("hidden");const b=await Bt(),F={source:T,nickname:g,rating:Number(h),message:x,ipHash:b,turnstileResponse:P||""},dt=await fetch(ut,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(F)}),$t=await dt.json();if(!dt.ok)throw new Error($t.error||"Failed to submit rating");showNotification("Rating submitted successfully! It will be visible after moderation.","success"),setTimeout(l,1500),V.reset(),window.turnstile&&window.turnstile.reset()}catch(p){console.error("Error submitting rating:",p),O.textContent=p.message||"Failed to submit rating. Please try again.",O.classList.remove("hidden"),window.turnstile&&window.turnstile.reset()}finally{j&&(j.disabled=!1,j.innerHTML='<i class="fas fa-paper-plane mr-2"></i> Submit Review')}}}document.body.classList.add("preloading");document.addEventListener("DOMContentLoaded",async()=>{await Mt.initialize()});function Dt(){const t=document.getElementById("preloader"),e=t.querySelector(".loading-progress"),s=t.querySelector(".loading-percentage");let a=0;const n=()=>{const i=100-a,o=Math.min(i*.1,Math.max(.2,Math.random()*.8));a=Math.min(100,a+o),e.style.width=`${a}%`;const r=a<100?a.toFixed(1):Math.round(a);s.textContent=`${r}%`,a<30?s.className="loading-percentage text-sm font-medium text-white/70":a<60?s.className="loading-percentage text-sm font-medium text-emerald-400/70":s.className="loading-percentage text-sm font-medium text-emerald-400",a<100?requestAnimationFrame(n):setTimeout(()=>{t.classList.add("hiding"),t.addEventListener("transitionend",()=>{t.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),et()},{once:!0})},1e3)};setTimeout(()=>{t&&document.body.contains(t)&&(console.log("Preloader safety timeout triggered"),t.classList.add("hiding"),setTimeout(()=>{t.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),et()},400))},5e3),setTimeout(()=>{requestAnimationFrame(n)},300)}function et(){const t=document.getElementById("language-switcher"),e=document.getElementById("language-dropdown");if(!t||!e)return;const s=t.cloneNode(!0);t.parentNode.replaceChild(s,t);const a=y.getCurrentLocale(),n=s.querySelector("span");if(n){const r={en:"English",ru:"Русский","pt-br":"Português"};n.textContent=r[a]||"English"}s.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),e.classList.contains("hidden")?(e.classList.remove("hidden"),s.classList.add("bg-white/10")):(e.classList.add("hidden"),s.classList.remove("bg-white/10"))}),e.querySelectorAll("button").forEach(r=>{r.addEventListener("click",u=>{u.preventDefault(),u.stopPropagation();const l=r.dataset.lang;if(y.setLocale(l),e.classList.add("hidden"),s.classList.remove("bg-white/10"),n){const f={en:"English",ru:"Русский","pt-br":"Português"};n.textContent=f[l]||"English"}q()})}),document.addEventListener("click",r=>{s.contains(r.target)||(e.classList.add("hidden"),s.classList.remove("bg-white/10"))});const o=()=>{const r=t.querySelector("span"),u={en:"English",ru:"Русский","pt-br":"Português"};r.textContent=u[y.currentLocale]||"English"};document.addEventListener("languageChanged",o)}document.addEventListener("DOMContentLoaded",()=>{try{Dt()}catch(i){console.error("Error initializing preloader:",i);const o=document.getElementById("preloader");o&&o.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),et()}Zt();const t=document.getElementById("accept-cookies"),e=document.getElementById("reject-cookies"),s=document.getElementById("cookie-consent");t&&t.addEventListener("click",()=>{a(),tt("cookie-consent","accepted",365)}),e&&e.addEventListener("click",()=>{a(),tt("cookie-consent","rejected",365)});function a(){s.classList.add("translate-y-full"),s.addEventListener("transitionend",()=>{s.style.display="none"},{once:!0})}yt(),setTimeout(()=>{Et()},300),Xt(),te(),ee(),st(),jt(),initializeMobileFilters();let n;window.addEventListener("resize",()=>{clearTimeout(n),n=setTimeout(()=>{st()},100)})});function yt(){try{console.log("Initializing sorting..."),document.querySelectorAll(".sort-option").forEach(e=>{e.addEventListener("click",s=>{s.preventDefault(),s.stopPropagation();const a=e.getAttribute("data-sort");if(console.log("Sort option clicked:",a),a){at(a);const n=document.getElementById("mobile-filters");n&&!n.classList.contains("hidden")&&(n.classList.add("hidden"),document.body.style.overflow="auto"),q()}})});const t=localStorage.getItem("currentSort")||"hot";console.log("Initial sort type:",t),at(t),console.log("Sorting initialized with type:",t)}catch(t){console.error("Error initializing sorting:",t)}}let v=[],N="",D=null,I=1;const U={mobile:4,tablet:6,desktop:9,wide:15},H=document.getElementById("about-modal"),Ft=document.getElementById("about-btn"),wt=document.getElementById("close-about"),_=document.getElementById("suggest-modal"),Ht=document.getElementById("suggest-btn"),Nt=document.getElementById("close-suggest"),Z=24*60*60*1e3;document.getElementById("mobile-filters-btn");document.getElementById("mobile-filters-modal");document.getElementById("close-mobile-filters");document.getElementById("mobile-filters-content");document.getElementById("reset-filters");document.getElementById("apply-filters");const Y=document.getElementById("active-filters-count"),_t={tablet:768};function st(){var a;const t=document.getElementById("filters-sidebar"),e=(a=document.getElementById("mobile-filters-btn"))==null?void 0:a.parentElement;if(!t||!e)return;const s=window.innerWidth<_t.tablet;t.classList.toggle("hidden",s),e.classList.toggle("hidden",!s)}function jt(){const t=document.getElementById("mobile-filters-btn"),e=document.getElementById("mobile-filters-modal"),s=e==null?void 0:e.querySelector(".flex.flex-col.bg-\\[\\#111\\]"),a=e==null?void 0:e.querySelector(".bg-\\[\\#0A0A0A\\]");if(!t||!e||!s||!a)return;function n(){e.classList.remove("hidden"),e.offsetHeight,a.classList.add("opacity-100"),s.classList.remove("translate-y-full"),document.body.classList.add("overflow-hidden")}function i(){a.classList.remove("opacity-100"),s.classList.add("translate-y-full"),setTimeout(()=>{e.classList.add("hidden"),document.body.classList.remove("overflow-hidden")},300)}t.addEventListener("click",l=>{l.preventDefault(),n()});const o=document.getElementById("close-mobile-filters");o&&o.addEventListener("click",l=>{l.preventDefault(),i()}),e.addEventListener("click",l=>{l.target===e&&i()}),document.addEventListener("keydown",l=>{l.key==="Escape"&&!e.classList.contains("hidden")&&i()});const r=document.getElementById("reset-filters");r&&r.addEventListener("click",l=>{l.preventDefault(),Ot(),St()});const u=document.getElementById("apply-filters");u&&u.addEventListener("click",l=>{l.preventDefault(),i(),q()})}function St(){const t=Ut();Y&&(t>0?(Y.textContent=`${t} active`,Y.classList.remove("hidden")):Y.classList.add("hidden"))}function Ut(){let t=0;return N&&t++,D&&t++,localStorage.getItem("currentSort")&&t++,t}function Ot(){N="",D=null,localStorage.removeItem("currentSort"),document.querySelectorAll(".status-filter-btn div").forEach(t=>{t.classList.remove("bg-black/40")}),document.querySelectorAll(".games-filter-btn div").forEach(t=>{t.classList.remove("border-emerald-500/20","bg-black/40"),t.classList.add("border-white/5","bg-black/20")}),document.querySelectorAll(".sort-option").forEach(t=>{t.classList.remove("bg-white/10","text-white"),t.classList.add("text-white/70")}),q()}async function Et(){try{v=(await(await fetch("data/resources.json")).json()).sources;const s="hydra_ratings_cache",a=5*60*1e3,n=()=>{try{const r=localStorage.getItem(s);if(!r)return null;const{timestamp:u,data:l}=JSON.parse(r);if(Date.now()-u<a)return l}catch(r){console.error("Error reading from cache:",r)}return null},i=r=>{try{localStorage.setItem(s,JSON.stringify({timestamp:Date.now(),data:r}))}catch(u){console.error("Error saving to cache:",u)}},o=n();if(o){console.log("Using cached ratings data"),v.forEach(r=>{const u=o[r.title]||{avg:0,total:0};r.rating={avg:parseFloat(u.avg)||0,total:parseInt(u.total)||0}});return}console.log("Fetching fresh ratings data...");try{const r=[...new Set(v.map(m=>m.title))];console.log(`Fetching ratings for ${r.length} sources`);const u=`https://libraryratingsdb.zxcsixx.workers.dev/api/ratings?batch=true&sources=${r.map(encodeURIComponent).join(",")}`;console.log("Batch API URL:",u);const l=await fetch(u);if(console.log("Batch response status:",l.status),!l.ok)throw new Error(`HTTP error! status: ${l.status}`);const f=await l.json();console.log("Received batch ratings data"),i(f),v.forEach(m=>{const w=f[m.title]||{avg:0,total:0};m.rating={avg:parseFloat(w.avg)||0,total:parseInt(w.total)||0}}),console.log("All ratings loaded via batch endpoint")}catch(r){console.error("Error in batch ratings fetch:",r),v.forEach(u=>{u.rating={avg:0,total:0}})}yt(),q(),zt()}catch(t){console.error("Error loading sources:",t);const e=document.getElementById("sources-container");e&&(e.innerHTML=`
    ...
    `;const l=(T=r.querySelector("[data-rating-stars-container]"))==null?void 0:T.parentElement;l&&(l.addEventListener("click",$=>{$.stopPropagation(),Rt(t)}),Tt(r,t.rating));const f=r.querySelector(".install-btn"),m=r.querySelector(".copy-btn");return m&&m.addEventListener("click",async()=>{const $=async()=>{await vt(t.url,"copy")&&(navigator.clipboard.writeText(t.url),m.innerHTML='<i class="fas fa-check text-[10px]"></i> '+y.t("sourceCard.copied"),setTimeout(()=>{m.innerHTML='<i class="fas fa-copy text-[10px]"></i> '+y.t("sourceCard.copy")},2e3))},M=t.title==="CPGRepacks";n||M?(M?ft:gt)(A=>{A&&$()}):$()}),f&&f.addEventListener("click",async()=>{const $=async()=>{xt(f,"loading");const C=await vt(t.url,"install");if(xt(f,C?"success":"rate-limited"),C){const A=encodeURIComponent(t.url);window.location.href=`hydralauncher://install-source?urls=${A}`}},M=t.title==="CPGRepacks";t.status.includes("Use At Your Own Risk")||M?(M?ft:gt)(A=>{A&&$()}):$()}),ct(t.url,t.stats),r.dataset.copies=i.copies||0,r.dataset.installs=i.installs||0,r.dataset.activity=o,r}let K="wide";function Lt(){const t=window.innerWidth;return t>=1536?"wide":t>=1024?"desktop":t>=640?"tablet":"mobile"}let pt;window.addEventListener("resize",()=>{clearTimeout(pt),pt=setTimeout(()=>{st();const t=Lt();if(K!==t){const s=lt(),a=Q(),n=Math.ceil(s.length/a);if(K==="wide"&&t==="desktop"){const i=(I-1)*U.desktop;I=Math.floor(i/U.desktop)+1}K=t,I=Math.min(Math.max(1,I),n),q(s),a<Q()&&window.scrollTo({top:0,behavior:"smooth"})}},100)});function ht(t,e){return console.log(`Sorting sources by: ${e}`),[...t].sort((s,a)=>{var n,i,o,r,u,l,f,m,w,S,E,L;try{switch(e){case"hot":const T=((n=s.stats)==null?void 0:n.recentActivity)||0;return(((i=a.stats)==null?void 0:i.recentActivity)||0)-T;case"new":return new Date(a.addedDate||0)-new Date(s.addedDate||0);case"most-copies":const M=((o=s.stats)==null?void 0:o.copies)||0;return(((r=a.stats)==null?void 0:r.copies)||0)-M;case"most-installs":const A=((u=s.stats)==null?void 0:u.installs)||0;return(((l=a.stats)==null?void 0:l.installs)||0)-A;case"top-rated":console.log(`Comparing ratings: ${s.title} (${((f=s.rating)==null?void 0:f.avg)||0}) vs ${a.title} (${((m=a.rating)==null?void 0:m.avg)||0})`),console.log(`Full rating data for ${s.title}:`,s.rating),console.log(`Full rating data for ${a.title}:`,a.rating);const B=((w=s.rating)==null?void 0:w.avg)||0,R=((S=a.rating)==null?void 0:S.avg)||0;if(B===R){const k=((E=s.rating)==null?void 0:E.total)||0,J=((L=a.rating)==null?void 0:L.total)||0;return console.log(`Ratings equal (${B}), comparing counts: ${k} vs ${J}`),J-k}return console.log(`Sorting by rating: ${R} - ${B} = ${R-B}`),R-B;case"name-asc":return(s.title||"").localeCompare(a.title||"");case"name-desc":return(a.title||"").localeCompare(s.title||"");default:return 0}}catch(T){return console.error("Error in sort function:",T),console.error("Sort type:",e),console.error("Source A:",s),console.error("Source B:",a),0}})}function at(t){var e;try{console.log("Updating active sort UI for:",t);const s=document.querySelector(".mobile-sort-button span");if(s){const n=((e=document.querySelector(`.sort-option[data-sort="${t}"]`))==null?void 0:e.textContent)||"Sort";s.textContent=n}document.querySelectorAll(".sort-option").forEach(n=>{const i=n.getAttribute("data-sort")===t;n.classList.toggle("bg-white/10",i),n.classList.toggle("text-white",i),n.classList.toggle("text-white/70",!i);const o=n.querySelector("i.fa-check");o&&(o.style.opacity=i?"1":"0")});const a=document.getElementById("sort-dropdown");a&&!a.classList.contains("hidden")&&a.classList.add("hidden"),localStorage.setItem("currentSort",t)}catch(s){console.error("Error updating sort UI:",s)}}function q(t=null){const e=document.getElementById("sources-container");if(e){e.innerHTML="";try{let s=t||lt();console.log(`Displaying ${s.length} filtered sources`);const a=localStorage.getItem("currentSort")||"hot";if(console.log("Current sort type:",a),a){console.log("Before sorting - first few sources:",s.slice(0,3).map(l=>{var f,m;return{title:l.title,rating:l.rating,hasRating:!!((f=l.rating)!=null&&f.avg||(m=l.rating)!=null&&m.total)}}));try{s=ht(s,a),console.log("After sorting - first few sources:",s.slice(0,3).map(l=>{var f,m;return{title:l.title,rating:l.rating,sortKey:a==="top-rated"?`${((f=l.rating)==null?void 0:f.avg)||0} (${((m=l.rating)==null?void 0:m.total)||0} ratings)`:l[a]||"N/A"}})),at(a)}catch(l){console.error("Error during sorting:",l),s=ht(s,"hot")}}K=Lt();const n=Q(),i=Math.ceil(s.length/n);I=Math.min(Math.max(1,I),i);const o=(I-1)*n,r=Math.min(o+n,s.length);s.slice(o,r).forEach(l=>{const f=Gt(l);e.appendChild(f)}),Jt(i),y.updatePageContent()}catch(s){console.error("Error in displaySources:",s),e.innerHTML=`
            <div class="col-span-full text-center py-10 px-4">
                <div class="text-red-400 text-4xl mb-4">
                    <i class="fas fa-exclamation-triangle"></i>
                </div>
        ...
        `}}}function Q(){const t=window.innerWidth;return t>=2560?U.wide:t>=1024?U.desktop:t>=640?U.tablet:U.mobile}function Jt(t){const e=document.getElementById("pagination");if(!e)return;let s="";s+=`
    ...
    `,e.innerHTML=s}window.changePage=function(t){if(t<1)return;const e=lt(),s=Math.ceil(e.length/Q());t>s||(I=t,q(e),window.scrollTo({top:0,behavior:"smooth"}))};document.querySelectorAll(".status-filter-btn").forEach(t=>{t.addEventListener("click",()=>{const e=t.dataset.status;document.querySelectorAll(".status-filter-btn").forEach(s=>{s.querySelector("div").classList.remove("bg-black/40")}),N===e?N="":(N=e,t.querySelector("div").classList.add("bg-black/40")),I=1,q(),Ct(),dispatchFiltersChanged()})});document.querySelectorAll(".games-filter-btn").forEach(t=>{t.addEventListener("click",()=>{const e=parseInt(t.dataset.min),s=parseInt(t.dataset.max);if(document.querySelectorAll(".games-filter-btn div").forEach(a=>{a.classList.remove("border-emerald-500/20","bg-black/40"),a.classList.add("border-white/5","bg-black/20")}),D&&D.min===e&&D.max===s)D=null;else{D={min:e,max:s};const a=t.querySelector("div");a.classList.remove("border-white/5","bg-black/20"),a.classList.add("border-emerald-500/20","bg-black/40")}I=1,q(),Ct(),St(),dispatchFiltersChanged()})});function Ct(){const t={};v.forEach(e=>{e.status.forEach(s=>{t[s]=(t[s]||0)+1})}),document.querySelectorAll(".status-filter-btn").forEach(e=>{const s=e.dataset.status,a=e.querySelector(".text-white\\/40");a&&(a.textContent=t[s]||0)}),document.querySelectorAll(".games-filter-btn").forEach(e=>{const s=parseInt(e.dataset.min),a=parseInt(e.dataset.max),n=v.filter(r=>{const u=parseInt(r.gamesCount);return u>=s&&u<=a}).length,i=e.querySelector(".text-white\\/40");i&&(i.textContent=n);const o=e.querySelector(".bg-emerald-500\\/50");if(o){const r=Math.max(...Array.from(document.querySelectorAll(".games-filter-btn")).map(l=>{const f=parseInt(l.dataset.min),m=parseInt(l.dataset.max);return v.filter(w=>{const S=parseInt(w.gamesCount);return S>=f&&S<=m}).length})),u=r>0?n/r*100:0;o.style.width=`${u}%`}})}let z="";document.getElementById("search").addEventListener("input",t=>{z=t.target.value.toLowerCase(),I=1,q()});function lt(){return v.filter(t=>{const e=!z||t.title.toLowerCase().includes(z)||t.description.toLowerCase().includes(z)||t.url.toLowerCase().includes(z),s=!N||t.status.includes(N),a=parseInt(t.gamesCount),n=!D||a>=D.min&&a<=D.max;return e&&s&&n})}const kt=document.getElementById("sort-dropdown-btn"),X=document.getElementById("sort-dropdown"),Vt=document.getElementById("current-sort");kt.addEventListener("click",()=>{X.classList.toggle("hidden")});document.addEventListener("click",t=>{!kt.contains(t.target)&&!X.contains(t.target)&&X.classList.add("hidden")});document.querySelectorAll(".sort-option").forEach(t=>{t.addEventListener("click",()=>{const e=t.dataset.sort;switch(Vt.textContent=t.textContent.trim(),X.classList.add("hidden"),e){case"high-to-low":sortGamesFilters(!1);break;case"low-to-high":sortGamesFilters(!0);break;case"most-sources":Wt();break;case"most-popular":Yt();break}})});function Wt(){const t=document.querySelector(".games-filter-btn").parentNode,e=Array.from(t.querySelectorAll(".games-filter-btn"));e.sort((s,a)=>{const n=parseInt(s.querySelector(".text-white\\/40").textContent);return parseInt(a.querySelector(".text-white\\/40").textContent)-n}),e.forEach(s=>s.remove()),e.forEach(s=>t.appendChild(s))}function Yt(){const t=JSON.parse(localStorage.getItem("sourceStats")||"{}"),e=document.querySelector(".games-filter-btn").parentNode,s=Array.from(e.querySelectorAll(".games-filter-btn"));s.sort((a,n)=>{var m,w;const i=((m=v.find(S=>S.gamesCount===a.dataset.min))==null?void 0:m.url)||"",o=((w=v.find(S=>S.gamesCount===n.dataset.min))==null?void 0:w.url)||"",r=t[i]||{installs:0,copies:0},u=t[o]||{installs:0,copies:0},l=r.installs+r.copies;return u.installs+u.copies-l}),s.forEach(a=>a.remove()),s.forEach(a=>e.appendChild(a))}function Kt(t){const e=new Date(t),a=Math.abs(new Date-e),n=Math.floor(a/(1e3*60*60*24)),i=y.t.bind(y);if(n===0)return i("common.date.today");if(n===1)return i("common.date.yesterday");if(n<30)return i("common.date.daysAgo",{days:n});{const o={year:"numeric",month:"short",day:"numeric"};return e.toLocaleDateString(y.getCurrentLocale(),o)}}document.addEventListener("DOMContentLoaded",()=>{Et(),sortGamesFilters(!1)});Ft.addEventListener("click",()=>{H.classList.remove("hidden"),document.body.style.overflow="hidden"});wt.addEventListener("click",()=>{H.classList.add("hidden"),document.body.style.overflow=""});H.addEventListener("click",t=>{t.target===H&&wt.click()});document.addEventListener("keydown",t=>{t.key==="Escape"&&!H.classList.contains("hidden")&&(H.classList.add("hidden"),document.body.style.overflow="")});Ht.addEventListener("click",()=>{_.classList.remove("hidden"),document.body.style.overflow="hidden"});Nt.addEventListener("click",()=>{_.classList.add("hidden"),document.body.style.overflow=""});_.addEventListener("click",t=>{t.target===_&&(_.classList.add("hidden"),document.body.style.overflow="")});document.addEventListener("keydown",t=>{t.key==="Escape"&&(H.classList.contains("hidden")||(H.classList.add("hidden"),document.body.style.overflow=""),_.classList.contains("hidden")||(_.classList.add("hidden"),document.body.style.overflow=""))});function tt(t,e,s){const a=new Date;a.setTime(a.getTime()+s*24*60*60*1e3),document.cookie=`${t}=${e};expires=${a.toUTCString()};path=/`}function It(t){const e=t+"=",s=document.cookie.split(";");for(let a=0;a<s.length;a++){let n=s[a];for(;n.charAt(0)===" ";)n=n.substring(1,n.length);if(n.indexOf(e)===0)return n.substring(e.length,n.length)}return null}async function vt(t,e){try{const s=t.replace(/[^a-zA-Z0-9]/g,"_"),a=`${e}_${s}`;if(It(a))return!0;let n={installs:0,copies:0,activity:[],recentActivity:0};try{const o=new Promise((E,L)=>setTimeout(()=>L(new Error("Firebase request timed out")),3e3)),r=rt(ot,`sources/${s}/stats`),u=it(r),f=(await Promise.race([u,o])).val()||{installs:0,copies:0,activity:[]},m=Date.now(),S=(Array.isArray(f.activity)?f.activity:[]).filter(E=>m-E<Z);S.push(m),n={installs:parseInt(f.installs||0)+(e==="install"?1:0),copies:parseInt(f.copies||0)+(e==="copy"?1:0),activity:S,recentActivity:S.length,lastUpdated:m},await Promise.race([bt(r,n),new Promise((E,L)=>setTimeout(()=>L(new Error("Update timed out")),3e3))]);try{const E=localStorage.getItem("sourceStats");if(E){const L=JSON.parse(E);L[s]||(L[s]={}),L[s].stats=n,localStorage.setItem("sourceStats",JSON.stringify(L)),localStorage.setItem("statsLastUpdated",m.toString())}}catch{}}catch{console.log("Firebase update failed, continuing with local data");const r=v.find(u=>u.url===t);if(r&&r.stats){n={installs:parseInt(r.stats.installs||0)+(e==="install"?1:0),copies:parseInt(r.stats.copies||0)+(e==="copy"?1:0),activity:[],recentActivity:1,lastUpdated:Date.now()};try{const u=localStorage.getItem("sourceStats");if(u){const l=JSON.parse(u);l[s]||(l[s]={}),l[s].stats=n,localStorage.setItem("sourceStats",JSON.stringify(l)),localStorage.setItem("statsLastUpdated",Date.now().toString())}}catch{}}}tt(a,"true",e==="install"?.003472222:347222e-9),ct(t,n);const i=v.findIndex(o=>o.url===t);return i!==-1&&(v[i].stats=n),!0}catch{return!1}}function ct(t,e=null){const s=document.querySelectorAll(".source-card"),a=Array.from(s).find(n=>n.dataset.url===t);if(a){const n=a.querySelector(".source-stats");if(n){const i=parseInt((e==null?void 0:e.installs)||0),o=parseInt((e==null?void 0:e.copies)||0),r=parseInt((e==null?void 0:e.recentActivity)||0);n.innerHTML=`
                <span class="flex items-center gap-1.5">
