import{c as ye,i as y,r as K,d as Q,g as X,u as ce}from"./index-UtUFAJBP.js";const se="https://libraryratingsdb.zxcsixx.workers.dev/api/ratings";function we(e){const t=new Date(e),a=Math.floor((new Date-t)/1e3);let n=Math.floor(a/31536e3);return n>=1?n+" year"+(n===1?"":"s")+" ago":(n=Math.floor(a/2592e3),n>=1?n+" month"+(n===1?"":"s")+" ago":(n=Math.floor(a/86400),n>=1?n+" day"+(n===1?"":"s")+" ago":(n=Math.floor(a/3600),n>=1?n+" hour"+(n===1?"":"s")+" ago":(n=Math.floor(a/60),n>=1?n+" minute"+(n===1?"":"s")+" ago":"just now"))))}function P(e){return String(e).replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Se(e){let t=localStorage.getItem("hydra_rating_hash");return t||(t=Math.random().toString(36).slice(2)+Date.now(),localStorage.setItem("hydra_rating_hash",t)),t}function Le(e){document.querySelectorAll(".rating-modal").forEach(d=>d.remove());const t=document.createElement("div");t.className="rating-modal fixed inset-0 z-[9999] flex items-center justify-center p-2 sm:p-4",t.innerHTML=`
  ...
  `,document.body.appendChild(t),document.body.style.overflow="hidden",t.querySelector(".close-rating-modal").onclick=n,t.querySelector(".fixed.inset-0").onclick=d=>{d.target===t.querySelector(".fixed.inset-0")&&n()};const s=t.querySelector('textarea[name="comment"]'),a=t.querySelector(".char-count");if(s&&a){const d=()=>{a.textContent=s.value.length};s.addEventListener("input",d),d(),t._cleanupCharCounter=()=>{s.removeEventListener("input",d)}}function n(){t._cleanupCharCounter&&t._cleanupCharCounter(),t.remove(),document.body.style.overflow=""}let r=1,i="recent",o=[],c=0;async function l(d=1){const v=t.querySelector("#rating-comments-list");v.innerHTML='<div class="flex justify-center items-center py-6"><div class="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-emerald-400"></div></div>';try{const f=`${se}?source=${encodeURIComponent(e.title)}`;console.log("Fetching ratings from:",f);const x=await fetch(f);if(!x.ok)throw new Error(`HTTP error! status: ${x.status}`);const m=await x.json();console.log("Received ratings data:",m),o=Array.isArray(m.comments)?m.comments:[],c=m.total||0,console.log("Processed comments:",o),E(m.avg||0),t.querySelector("#rating-modal-avg").textContent=m.avg?`${m.avg} / 5`:"No ratings yet",t.querySelector("#rating-modal-total").textContent=m.total?`(${m.total} ${m.total===1?"review":"reviews"})`:"",u(d)}catch(f){console.error("Error loading comments:",f),v.innerHTML=`
      ...
      `,I>1){let p='<div class="flex flex-wrap gap-2 justify-center mt-6 pt-4 border-t border-white/5">';S>1&&(p+=`
          ...
          </button>`);const $=5;let j=Math.max(1,S-Math.floor($/2)),W=Math.min(I,j+$-1);W-j+1<$&&(j=Math.max(1,W-$+1));for(let B=j;B<=W;B++)p+=`
          <button class="px-3 py-1.5 min-w-[2.25rem] text-xs rounded-md transition-all duration-200 ${B===S?"bg-emerald-500/90 text-white font-medium shadow-lg shadow-emerald-500/20":"bg-black/40 text-white/70 border border-white/5 hover:bg-black/60 hover:text-white"}" data-page="${B}">
            ${B}
          </button>`;S<I&&(p+=`
          <button class="px-3 py-1.5 text-xs rounded-md bg-black/40 text-white/70 border border-white/5 hover:bg-black/60 hover:text-white transition-colors" 
                  data-page="${S+1}">
            Next <i class="fas fa-chevron-right ml-1"></i>
          </button>`),p+="</div>",v.insertAdjacentHTML("afterend",p),document.querySelectorAll("#rating-comments-list + div [data-page]").forEach(B=>{B.addEventListener("click",be=>{be.preventDefault();const H=parseInt(B.dataset.page);!isNaN(H)&&H>=1&&H<=I&&(u(H),window.scrollTo({top:v.offsetTop-20,behavior:"smooth"}))})})}}function g(){const d=t.querySelector("#rating-sort-select");d&&(d.value=i,d.onchange=v=>{i=v.target.value,r=1,u(r)})}g(),l();function E(d){const v=t.querySelector("#rating-modal-stars");v.innerHTML="";for(let f=1;f<=5;++f)f<=Math.round(d)?v.innerHTML+='<svg class="inline-block w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clip-rule="evenodd" /></svg>':v.innerHTML+='<svg class="inline-block w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" /></svg>'}l();const w=t.querySelector("#submit-rating-form"),h=t.querySelector("#rating-form-error");w.onsubmit=async d=>{d.preventDefault(),h.textContent="",h.className="text-sm text-rose-400 mt-2";const v=new FormData(w),f=(v.get("nickname")||"").trim(),x=v.get("rating"),m=(v.get("comment")||"").trim();if(!f){h.textContent="Please enter a nickname.";return}if(!x||isNaN(x)||x<1||x>5){h.textContent="Please select a valid rating between 1 and 5.";return}if(!m){h.textContent="Please enter a message.";return}if(m.split(/\s+/).length<3){h.textContent="Message must be at least 3 words long.";return}const L=Se(),I=`hydra_rating_${e.title}_${L}`;if(localStorage.getItem(I)){h.textContent="You have already submitted a review for this source.";return}try{const S=await fetch(se,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({source:e.title,nickname:f,rating:Number(x),message:m,ipHash:L})});if(!S.ok){const M=await S.text();h.textContent=M||"Failed to submit.";return}localStorage.setItem(I,"1"),w.reset(),h.textContent="Submitted for moderation!",window.updateSourceCardRating&&await window.updateSourceCardRating(e.title),l(r)}catch{h.textContent="Failed to submit."}}}document.body.classList.add("preloading");document.addEventListener("DOMContentLoaded",async()=>{await ye.initialize()});function ke(){const e=document.getElementById("preloader"),t=e.querySelector(".loading-progress"),s=e.querySelector(".loading-percentage");let a=0;const n=()=>{const r=100-a,i=Math.min(r*.1,Math.max(.2,Math.random()*.8));a=Math.min(100,a+i),t.style.width=`${a}%`;const o=a<100?a.toFixed(1):Math.round(a);s.textContent=`${o}%`,a<30?s.className="loading-percentage text-sm font-medium text-white/70":a<60?s.className="loading-percentage text-sm font-medium text-emerald-400/70":s.className="loading-percentage text-sm font-medium text-emerald-400",a<100?requestAnimationFrame(n):setTimeout(()=>{e.classList.add("hiding"),e.addEventListener("transitionend",()=>{e.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),J()},{once:!0})},1e3)};setTimeout(()=>{e&&document.body.contains(e)&&(console.log("Preloader safety timeout triggered"),e.classList.add("hiding"),setTimeout(()=>{e.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),J()},400))},5e3),setTimeout(()=>{requestAnimationFrame(n)},300)}function J(){const e=document.getElementById("language-switcher"),t=document.getElementById("language-dropdown");if(!e||!t)return;const s=e.cloneNode(!0);e.parentNode.replaceChild(s,e);const a=y.getCurrentLocale(),n=s.querySelector("span");if(n){const o={en:"English",ru:"Русский","pt-br":"Português"};n.textContent=o[a]||"English"}s.addEventListener("click",o=>{o.preventDefault(),o.stopPropagation(),t.classList.contains("hidden")?(t.classList.remove("hidden"),s.classList.add("bg-white/10")):(t.classList.add("hidden"),s.classList.remove("bg-white/10"))}),t.querySelectorAll("button").forEach(o=>{o.addEventListener("click",c=>{c.preventDefault(),c.stopPropagation();const l=o.dataset.lang;if(y.setLocale(l),t.classList.add("hidden"),s.classList.remove("bg-white/10"),n){const u={en:"English",ru:"Русский","pt-br":"Português"};n.textContent=u[l]||"English"}C()})}),document.addEventListener("click",o=>{s.contains(o.target)||(t.classList.add("hidden"),s.classList.remove("bg-white/10"))});const i=()=>{const o=e.querySelector("span"),c={en:"English",ru:"Русский","pt-br":"Português"};o.textContent=c[y.currentLocale]||"English"};document.addEventListener("languageChanged",i)}document.addEventListener("DOMContentLoaded",()=>{try{ke()}catch(r){console.error("Error initializing preloader:",r);const i=document.getElementById("preloader");i&&i.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),J()}je();const e=document.getElementById("accept-cookies"),t=document.getElementById("reject-cookies"),s=document.getElementById("cookie-consent");e&&e.addEventListener("click",()=>{a(),V("cookie-consent","accepted",365)}),t&&t.addEventListener("click",()=>{a(),V("cookie-consent","rejected",365)});function a(){s.classList.add("translate-y-full"),s.addEventListener("transitionend",()=>{s.style.display="none"},{once:!0})}de(),setTimeout(()=>{ge()},300),ze(),Ue(),Oe(),Y(),Ae(),initializeMobileFilters();let n;window.addEventListener("resize",()=>{clearTimeout(n),n=setTimeout(()=>{Y()},100)})});function de(){try{console.log("Initializing sorting..."),document.querySelectorAll(".sort-option").forEach(t=>{t.addEventListener("click",s=>{s.preventDefault(),s.stopPropagation();const a=t.getAttribute("data-sort");if(console.log("Sort option clicked:",a),a){Z(a);const n=document.getElementById("mobile-filters");n&&!n.classList.contains("hidden")&&(n.classList.add("hidden"),document.body.style.overflow="auto"),C()}})});const e=localStorage.getItem("currentSort")||"hot";console.log("Initial sort type:",e),Z(e),console.log("Sorting initialized with type:",e)}catch(e){console.error("Error initializing sorting:",e)}}let b=[],T="",A=null,k=1;const R={mobile:4,tablet:6,desktop:9,wide:15},q=document.getElementById("about-modal"),Ee=document.getElementById("about-btn"),ue=document.getElementById("close-about"),F=document.getElementById("suggest-modal"),Ce=document.getElementById("suggest-btn"),Ie=document.getElementById("close-suggest"),O=24*60*60*1e3;document.getElementById("mobile-filters-btn");document.getElementById("mobile-filters-modal");document.getElementById("close-mobile-filters");document.getElementById("mobile-filters-content");document.getElementById("reset-filters");document.getElementById("apply-filters");const z=document.getElementById("active-filters-count"),$e={tablet:768};function Y(){var a;const e=document.getElementById("filters-sidebar"),t=(a=document.getElementById("mobile-filters-btn"))==null?void 0:a.parentElement;if(!e||!t)return;const s=window.innerWidth<$e.tablet;e.classList.toggle("hidden",s),t.classList.toggle("hidden",!s)}function Ae(){const e=document.getElementById("mobile-filters-btn"),t=document.getElementById("mobile-filters-modal"),s=t==null?void 0:t.querySelector(".flex.flex-col.bg-\\[\\#111\\]"),a=t==null?void 0:t.querySelector(".bg-\\[\\#0A0A0A\\]");if(!e||!t||!s||!a)return;function n(){t.classList.remove("hidden"),t.offsetHeight,a.classList.add("opacity-100"),s.classList.remove("translate-y-full"),document.body.classList.add("overflow-hidden")}function r(){a.classList.remove("opacity-100"),s.classList.add("translate-y-full"),setTimeout(()=>{t.classList.add("hidden"),document.body.classList.remove("overflow-hidden")},300)}e.addEventListener("click",l=>{l.preventDefault(),n()});const i=document.getElementById("close-mobile-filters");i&&i.addEventListener("click",l=>{l.preventDefault(),r()}),t.addEventListener("click",l=>{l.target===t&&r()}),document.addEventListener("keydown",l=>{l.key==="Escape"&&!t.classList.contains("hidden")&&r()});const o=document.getElementById("reset-filters");o&&o.addEventListener("click",l=>{l.preventDefault(),Be(),me()});const c=document.getElementById("apply-filters");c&&c.addEventListener("click",l=>{l.preventDefault(),r(),C()})}function me(){const e=Me();z&&(e>0?(z.textContent=`${e} active`,z.classList.remove("hidden")):z.classList.add("hidden"))}function Me(){let e=0;return T&&e++,A&&e++,localStorage.getItem("currentSort")&&e++,e}function Be(){T="",A=null,localStorage.removeItem("currentSort"),document.querySelectorAll(".status-filter-btn div").forEach(e=>{e.classList.remove("bg-black/40")}),document.querySelectorAll(".games-filter-btn div").forEach(e=>{e.classList.remove("border-emerald-500/20","bg-black/40"),e.classList.add("border-white/5","bg-black/20")}),document.querySelectorAll(".sort-option").forEach(e=>{e.classList.remove("bg-white/10","text-white"),e.classList.add("text-white/70")}),C()}async function ge(){try{b=(await(await fetch("data/resources.json")).json()).sources,console.log("Fetching ratings for all sources..."),await Promise.all(b.map(async s=>{try{console.log(`Fetching rating for source: ${s.title}`);const a=`https://libraryratingsdb.zxcsixx.workers.dev/api/ratings?source=${encodeURIComponent(s.title)}`;console.log("API URL:",a);const n=await fetch(a);if(console.log("Response status:",n.status),!n.ok)throw new Error(`HTTP error! status: ${n.status}`);const r=await n.json();console.log(`Received rating for ${s.title}:`,JSON.stringify(r,null,2));const i=r&&typeof r.avg=="number"?parseFloat(r.avg):0,o=r&&typeof r.total=="number"?parseInt(r.total):0;return console.log(`Parsed rating for ${s.title}:`,{avg:i,total:o}),s.rating={avg:i,total:o},console.log(`Set rating for ${s.title}:`,JSON.stringify(s.rating,null,2)),{title:s.title,rating:s.rating}}catch(a){return console.error(`Error fetching rating for ${s.title}:`,a),s.rating={avg:0,total:0},console.log(`Set default rating for ${s.title} due to error`),{title:s.title,rating:{avg:0,total:0},error:a.message}}})),console.log("All ratings loaded, sources:",b.map(s=>({title:s.title,rating:s.rating}))),de(),C(),Pe()}catch(e){console.error("Error loading sources:",e);const t=document.getElementById("sources-container");t&&(t.innerHTML=`
            ...
            `)}finally{setTimeout(()=>{const e=document.getElementById("preloader");e&&(e.style.opacity="0",setTimeout(()=>{e.style.display="none",C()},300))},500)}}async function Pe(){try{const e=new Promise((r,i)=>setTimeout(()=>i(new Error("Firebase request timed out")),5e3)),t=K(Q,"sources"),s=X(t),n=(await Promise.race([s,e])).val();if(n){b=b.map(r=>{var g;const i=r.url.replace(/[^a-zA-Z0-9]/g,"_"),o=((g=n[i])==null?void 0:g.stats)||{installs:0,copies:0,activity:[]},c=Array.isArray(o.activity)?o.activity:[],l=Date.now(),u=c.filter(E=>l-E<O).length;return{...r,stats:{...o,recentActivity:u,activity:c}}}),b.forEach(r=>{te(r.url,r.stats)});try{localStorage.setItem("sourceStats",JSON.stringify(n)),localStorage.setItem("statsLastUpdated",Date.now().toString())}catch{}return!0}return!1}catch{try{const t=localStorage.getItem("sourceStats");if(t){const s=JSON.parse(t);return b=b.map(a=>{var l;const n=a.url.replace(/[^a-zA-Z0-9]/g,"_"),r=((l=s[n])==null?void 0:l.stats)||{installs:0,copies:0,activity:[]},i=Array.isArray(r.activity)?r.activity:[],o=Date.now(),c=i.filter(u=>o-u<O).length;return{...a,stats:{...r,recentActivity:c,activity:i}}}),await Promise.all(b.map(async a=>{try{const r=await(await fetch(`https://libraryratingsdb.zxcsixx.workers.dev/api/ratings?source=${encodeURIComponent(a.title)}&page=1`)).json();ratingsMap[a.title]={avg:typeof r.avg=="number"?r.avg:null,total:typeof r.total=="number"?r.total:0}}catch{ratingsMap[a.title]={avg:null,total:0}}})),console.log("Using cached stats from localStorage"),!0}}catch{}return b=b.map(t=>({...t,stats:{installs:0,copies:0,recentActivity:0,activity:[]}})),!1}}function ae(e){document.querySelector("main").classList.add("blur-sm","transition-all","duration-200");const t=y.t.bind(y),s=document.createElement("div");s.className="fixed inset-0 flex items-center justify-center z-50 animate-fade-in p-4",s.innerHTML=`
    ...
    `,document.body.appendChild(s);const a=()=>{document.querySelector("main").classList.remove("blur-sm"),s.remove()};s.querySelector(".cancel-btn").addEventListener("click",()=>{a(),e(!1)}),s.querySelector(".proceed-btn").addEventListener("click",()=>{a(),e(!0)}),s.querySelector(".fixed.inset-0").addEventListener("click",n=>{n.target===n.currentTarget&&(a(),e(!1))})}function qe(e){var E,w,h,d,v;const t=y.getCurrentLocale(),a=(((E=y.translations[t])==null?void 0:E.sources)||{})[e.title]||{title:e.title,description:e.description},n=e.status.includes("Use At Your Own Risk"),r=e.stats||{installs:0,copies:0,recentActivity:0},i=parseInt(r.recentActivity||0),o=document.createElement("div");o.className="source-card animate-fade-in rounded-xl",o.dataset.url=e.url,o.dataset.name=e.title,o.dataset.copies=((w=e.stats)==null?void 0:w.copies)||0,o.dataset.installs=((h=e.stats)==null?void 0:h.installs)||0,o.dataset.activity=((d=e.stats)==null?void 0:d.recentActivity)||0;const c=e.status.map(f=>{const x=f.toLowerCase().replace(/\s+/g,"-"),m={trusted:{color:"emerald",icon:"shield",key:"trusted",customClass:"bg-emerald-500/10 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20 hover:border-emerald-500/30 "},"safe-for-use":{color:"blue",icon:"check-circle",key:"safeForUse",text:"Safe for Use",customClass:"bg-blue-500/10 border-blue-500/20 text-blue-400 hover:bg-blue-500/20 hover:border-blue-500/30"},"use-at-your-own-risk":{color:"red",icon:"exclamation-triangle",key:"useAtOwnRisk",customClass:"bg-red-500/10 border-red-500/20 text-red-400 hover:bg-red-500/20 hover:border-red-500/30 "},"works-in-russia":{color:"teal",icon:"globe-europe",key:"worksInRussia",text:"Works in Russia",customClass:"bg-teal-500/10 border-teal-500/20 text-teal-400 hover:bg-teal-500/20 hover:border-teal-500/30 "},nsfw:{color:"purple",icon:"exclamation-circle",text:"NSFW",customClass:"bg-purple-500/10 border-purple-500/20 text-purple-400 hover:bg-purple-500/20 hover:border-purple-500/30 "}}[x]||{color:"gray",icon:"circle",key:x},L=`inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs backdrop-blur-sm status-badge ${m.customClass||""}`,I=`bg-${m.color}-500/10 border border-${m.color}-500/20 text-${m.color}-400`;return`
            <span class="${m.customClass?L:`${L} ${I}`}">
                <i class="fas fa-${m.icon} text-[10px]"></i>
                ${m.text||y.t(`status.${m.key}`)}
            </span>
        `}).join("");o.innerHTML=`
    ...
    `;const l=(v=o.querySelector("[data-rating-stars-container]"))==null?void 0:v.parentElement;l&&(l.addEventListener("click",f=>{f.stopPropagation(),Le(e)}),xe(o,e.rating));const u=o.querySelector(".install-btn"),g=o.querySelector(".copy-btn");return g&&g.addEventListener("click",async()=>{const f=async()=>{await ie(e.url,"copy")&&(navigator.clipboard.writeText(e.url),g.innerHTML='<i class="fas fa-check text-[10px]"></i> '+y.t("sourceCard.copied"),setTimeout(()=>{g.innerHTML='<i class="fas fa-copy text-[10px]"></i> '+y.t("sourceCard.copy")},2e3))},x=e.title==="CPGRepacks";n||x?(x?ne:ae)(L=>{L&&f()}):f()}),u&&u.addEventListener("click",async()=>{const f=async()=>{le(u,"loading");const m=await ie(e.url,"install");if(le(u,m?"success":"rate-limited"),m){const L=encodeURIComponent(e.url);window.location.href=`hydralauncher://install-source?urls=${L}`}},x=e.title==="CPGRepacks";e.status.includes("Use At Your Own Risk")||x?(x?ne:ae)(L=>{L&&f()}):f()}),te(e.url,e.stats),o.dataset.copies=r.copies||0,o.dataset.installs=r.installs||0,o.dataset.activity=i,o}let U="wide";function pe(){const e=window.innerWidth;return e>=1536?"wide":e>=1024?"desktop":e>=640?"tablet":"mobile"}let re;window.addEventListener("resize",()=>{clearTimeout(re),re=setTimeout(()=>{Y();const e=pe();if(U!==e){const s=ee(),a=_(),n=Math.ceil(s.length/a);if(U==="wide"&&e==="desktop"){const r=(k-1)*R.desktop;k=Math.floor(r/R.desktop)+1}U=e,k=Math.min(Math.max(1,k),n),C(s),a<_()&&window.scrollTo({top:0,behavior:"smooth"})}},100)});function oe(e,t){return console.log(`Sorting sources by: ${t}`),[...e].sort((s,a)=>{var n,r,i,o,c,l,u,g,E,w,h,d;try{switch(t){case"hot":const v=((n=s.stats)==null?void 0:n.recentActivity)||0;return(((r=a.stats)==null?void 0:r.recentActivity)||0)-v;case"new":return new Date(a.addedDate||0)-new Date(s.addedDate||0);case"most-copies":const x=((i=s.stats)==null?void 0:i.copies)||0;return(((o=a.stats)==null?void 0:o.copies)||0)-x;case"most-installs":const L=((c=s.stats)==null?void 0:c.installs)||0;return(((l=a.stats)==null?void 0:l.installs)||0)-L;case"top-rated":console.log(`Comparing ratings: ${s.title} (${((u=s.rating)==null?void 0:u.avg)||0}) vs ${a.title} (${((g=a.rating)==null?void 0:g.avg)||0})`),console.log(`Full rating data for ${s.title}:`,s.rating),console.log(`Full rating data for ${a.title}:`,a.rating);const S=((E=s.rating)==null?void 0:E.avg)||0,M=((w=a.rating)==null?void 0:w.avg)||0;if(S===M){const D=((h=s.rating)==null?void 0:h.total)||0,p=((d=a.rating)==null?void 0:d.total)||0;return console.log(`Ratings equal (${S}), comparing counts: ${D} vs ${p}`),p-D}return console.log(`Sorting by rating: ${M} - ${S} = ${M-S}`),M-S;case"name-asc":return(s.title||"").localeCompare(a.title||"");case"name-desc":return(a.title||"").localeCompare(s.title||"");default:return 0}}catch(v){return console.error("Error in sort function:",v),console.error("Sort type:",t),console.error("Source A:",s),console.error("Source B:",a),0}})}function Z(e){var t;try{console.log("Updating active sort UI for:",e);const s=document.querySelector(".mobile-sort-button span");if(s){const n=((t=document.querySelector(`.sort-option[data-sort="${e}"]`))==null?void 0:t.textContent)||"Sort";s.textContent=n}document.querySelectorAll(".sort-option").forEach(n=>{const r=n.getAttribute("data-sort")===e;n.classList.toggle("bg-white/10",r),n.classList.toggle("text-white",r),n.classList.toggle("text-white/70",!r);const i=n.querySelector("i.fa-check");i&&(i.style.opacity=r?"1":"0")});const a=document.getElementById("sort-dropdown");a&&!a.classList.contains("hidden")&&a.classList.add("hidden"),localStorage.setItem("currentSort",e)}catch(s){console.error("Error updating sort UI:",s)}}function C(e=null){const t=document.getElementById("sources-container");if(t){t.innerHTML="";try{let s=e||ee();console.log(`Displaying ${s.length} filtered sources`);const a=localStorage.getItem("currentSort")||"hot";if(console.log("Current sort type:",a),a){console.log("Before sorting - first few sources:",s.slice(0,3).map(l=>{var u,g;return{title:l.title,rating:l.rating,hasRating:!!((u=l.rating)!=null&&u.avg||(g=l.rating)!=null&&g.total)}}));try{s=oe(s,a),console.log("After sorting - first few sources:",s.slice(0,3).map(l=>{var u,g;return{title:l.title,rating:l.rating,sortKey:a==="top-rated"?`${((u=l.rating)==null?void 0:u.avg)||0} (${((g=l.rating)==null?void 0:g.total)||0} ratings)`:l[a]||"N/A"}})),Z(a)}catch(l){console.error("Error during sorting:",l),s=oe(s,"hot")}}U=pe();const n=_(),r=Math.ceil(s.length/n);k=Math.min(Math.max(1,k),r);const i=(k-1)*n,o=Math.min(i+n,s.length);s.slice(i,o).forEach(l=>{const u=qe(l);t.appendChild(u)}),Te(r),y.updatePageContent()}catch(s){console.error("Error in displaySources:",s),t.innerHTML=`
            <div class="col-span-full text-center py-10 px-4">
                <div class="text-red-400 text-4xl mb-4">
                    <i class="fas fa-exclamation-triangle"></i>
                </div>
