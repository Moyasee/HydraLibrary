  ...
  `,document.head.appendChild(i),e.querySelector(".close-rating-modal").onclick=S,e.querySelector(".fixed.inset-0").onclick=d=>{d.target===e.querySelector(".fixed.inset-0")&&S()};async function r(d,l="en"){return new Promise((g,m)=>{const h=JSON.stringify({translate:"rapidapi"}),y=new XMLHttpRequest;y.withCredentials=!0,y.addEventListener("readystatechange",function(){if(this.readyState===this.DONE)try{const b=JSON.parse(this.responseText);if(b&&b.translation)g(b.translation);else throw b&&b.status!==200?new Error(b.business_message||"Translation service error"):new Error("Invalid response from translation service")}catch(b){console.error("Translation error:",b,this.responseText),m(b)}}),y.addEventListener("error",()=>{m(new Error("Network error during translation"))});const D=`https://free-google-translator.p.rapidapi.com/external-api/free-google-translator?from=auto&to=${l}&query=${encodeURIComponent(d)}`;y.open("POST",D),y.setRequestHeader("x-rapidapi-key",""),y.setRequestHeader("x-rapidapi-host","free-google-translator.p.rapidapi.com"),y.setRequestHeader("Content-Type","application/json"),y.send(h)})}async function u(d){const l=d.target.closest(".translate-comment");if(!l||l.disabled)return;d.preventDefault(),d.stopPropagation();const g=l.closest(".border-b"),m=g?g.querySelector(".comment-text p"):null;if(!m){console.error("Could not find comment text element");return}let h=l.getAttribute("data-original");if(l.classList.contains("active")){h&&(m.textContent=h,l.innerHTML='<i class="fas fa-language text-xs"></i><span class="hidden sm:inline">Translate</span>',l.classList.remove("active"),l.title="Translate this review"),l.disabled=!1;return}h=m.textContent.trim(),l.setAttribute("data-original",h),l.innerHTML,l.innerHTML='<i class="fas fa-spinner fa-spin text-xs"></i>',l.title="Translating...",l.disabled=!0;try{const y=await r(h,"en");m.textContent=y,l.innerHTML='<i class="fas fa-undo text-xs"></i><span class="hidden sm:inline">Original</span>',l.classList.add("active"),l.title="Show original text"}catch(y){console.error("Translation failed:",y),m.textContent=h,l.innerHTML='<i class="fas fa-exclamation-triangle text-xs"></i><span class="hidden sm:inline">Error</span>',l.title="Error translating. Click to try again."}finally{l.disabled=!1}}const c=e.querySelector("#rating-comments-list");c&&c.addEventListener("click",u);const f=e.querySelector('textarea[name="comment"]'),p=e.querySelector(".char-count");if(f&&p){const d=()=>{p.textContent=f.value.length};f.addEventListener("input",d),d(),e._cleanupCharCounter=()=>{f.removeEventListener("input",d)}}function S(){e._cleanupCharCounter&&e._cleanupCharCounter(),e.remove(),document.body.style.overflow=""}const L=5*60*1e3;let x=1,C="recent",R=0,k=!1,T=0,E=t.title;function $(){const d=`ratings_${encodeURIComponent(E)}`,l=localStorage.getItem(d);if(!l)return null;try{const{data:g,timestamp:m}=JSON.parse(l);if(Date.now()-m<L)return g}catch(g){console.warn("Error reading cache:",g)}return null}function V(d){try{const l=`ratings_${encodeURIComponent(E)}`,g={data:d,timestamp:Date.now()};localStorage.setItem(l,JSON.stringify(g))}catch(l){console.warn("Error saving to cache:",l)}}async function q(d=1,l=!1){if(k)return;const g=e.querySelector("#rating-comments-list");if(!g)return;g.innerHTML='<div class="flex justify-center items-center py-6"><div class="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-emerald-400"></div></div>';const m=l?null:$();if(m){console.log("Using cached data for:",E),H(m,d),Date.now()-T>L&&U(!0);return}await U()}function H(d,l=1){const g=e.querySelector("#rating-comments-list");if(g){if(g.innerHTML="",d.avg!==void 0){const m=parseFloat(d.avg).toFixed(1),h=parseInt(d.total)||0,y=e.querySelector("#rating-modal-stars"),D=e.querySelector("#rating-modal-avg"),b=e.querySelector("#rating-modal-total");if(y){y.innerHTML="";const P=Math.round(parseFloat(d.avg));y.innerHTML="★".repeat(P)+"☆".repeat(5-P)}D&&(D.textContent=`${m} out of 5`),b&&(b.textContent=`${h} ${h===1?"review":"reviews"}`)}if(!d.comments||d.comments.length===0){g.innerHTML=`
      ...
      `;return}V(d),d.pagination&&(R=d.pagination.total||0,W()),d.comments.forEach(m=>{if(!m)return;const h=document.createElement("div");h.className="border-b border-gray-700 py-4";const y=m.timestamp?new Date(m.timestamp).getTime():Date.now();h.innerHTML=`
        <div class="flex items-start justify-between mb-2">
          ...
          <span class="text-xs text-gray-500">${Dt(y)}</span>
        </div>
    ...
    `;d.innerHTML=g}window.loadPage=d=>{if(!A||!A.comments)return;const l=Math.ceil(A.comments.length/10);d<1||d>l||(x=d,z())},e._vm={loadPage:window.loadPage};async function U(d=!1){var y,D;if(k)return;k=!0;const l=e.querySelector("#rating-comments-list");!d&&l&&(l.innerHTML='<div class="text-center py-4 text-gray-400">Loading comments...</div>');const g=`comments_${E}`,m=5*60*1e3,h=ft.get(g);if(h&&Date.now()-h.timestamp<m){A=h,z(),k=!1;return}try{const b=await fetch(`${gt}?source=${encodeURIComponent(E)}&all=true`);if(!b.ok)throw new Error(`Failed to fetch comments: ${b.statusText}`);const P=await b.json();A={...P,comments:P.comments||[],pagination:{total:((y=P.comments)==null?void 0:y.length)||0,page:1,totalPages:1,perPage:((D=P.comments)==null?void 0:D.length)||0}},ft.set(g,{...A,timestamp:Date.now()}),T=Date.now(),z()}catch(b){console.error("Error fetching comments:",b),h?(A=h,z()):!d&&l&&(l.innerHTML=`
          <div class="text-red-500 text-center py-4">
            Failed to load comments. Please try again later.
            ${b.message?`<div class="text-xs mt-1">${b.message}</div>`:""}
          </div>
        `)}finally{k=!1}}function z(){if(!A||!A.comments)return;const d=Mt([...A.comments],C),l=10,g=Math.ceil(d.length/l),m=(x-1)*l,h=d.slice(m,m+l);A.pagination={total:d.length,page:x,totalPages:g,perPage:l},H({...A,comments:h,pagination:A.pagination},x)}let A=null;function Mt(d,l){if(!d)return[];const g=[...d];switch(l){case"recent":return g.sort((m,h)=>{const y=m.timestamp?new Date(m.timestamp).getTime():0;return(h.timestamp?new Date(h.timestamp).getTime():0)-y});case"high":return g.sort((m,h)=>(h.rating||0)-(m.rating||0));case"low":return g.sort((m,h)=>(m.rating||0)-(h.rating||0));default:return g}}function qt(){const d=e.querySelector("#rating-sort-select");d&&d.addEventListener("change",l=>{C=l.target.value,x=1,z()})}qt(),q(),q();const Y=e.querySelector("#submit-rating-form"),G=e.querySelector("#rating-form-error"),j=Y.querySelector('button[type="submit"]');j.disabled=!0,Y.onsubmit=async d=>{d.preventDefault(),G.textContent="",G.className="text-sm text-rose-400 mt-2";try{const l=new FormData(Y),g=Object.fromEntries(l.entries()),m=parseInt(g.rating,10),h=g.comment.trim();if(isNaN(m)||m<1||m>5)throw new Error("Please select a valid rating");if(h&&h.split(/\s+/).length<3)throw new Error("Message must be at least 3 words");if(!g["cf-turnstile-response"])throw new Error("Please complete the CAPTCHA");const y=await Ft(),D=`hydra_rating_${nt}_${y}`;j.disabled=!0,j.innerHTML='<i class="fas fa-spinner fa-spin mr-2"></i> Submitting...',G.classList.add("hidden");const b={source:nt,rating:m,comment:h,ipHash:y,"cf-turnstile-response":g["cf-turnstile-response"]},P=await fetch(gt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(b)}),Pt=await P.json();if(!P.ok)throw new Error(Pt.error||"Failed to submit rating");showNotification("Rating submitted successfully! It will be visible after moderation.","success"),setTimeout(S,1500),Y.reset(),window.turnstile&&turnstile.reset()}catch(l){console.error("Error submitting rating:",l),G.textContent=l.message||"Failed to submit rating. Please try again.",G.classList.remove("hidden"),window.turnstile&&(window.turnstile.reset(),document.querySelector(".cf-turnstile-response").value="")}finally{j&&(j.disabled=!1,j.innerHTML='<i class="fas fa-paper-plane mr-2"></i> Submit Review')}}}document.body.classList.add("preloading");document.addEventListener("DOMContentLoaded",async()=>{await Bt.initialize()});function Nt(){const t=document.getElementById("preloader"),e=t.querySelector(".loading-progress"),s=t.querySelector(".loading-percentage");let n=0;const a=()=>{const o=100-n,i=Math.min(o*.1,Math.max(.2,Math.random()*.8));n=Math.min(100,n+i),e.style.width=`${n}%`;const r=n<100?n.toFixed(1):Math.round(n);s.textContent=`${r}%`,n<30?s.className="loading-percentage text-sm font-medium text-white/70":n<60?s.className="loading-percentage text-sm font-medium text-emerald-400/70":s.className="loading-percentage text-sm font-medium text-emerald-400",n<100?requestAnimationFrame(a):setTimeout(()=>{t.classList.add("hiding"),t.addEventListener("transitionend",()=>{t.remove(),document.body.classList.remove("preloading"),w.updatePageContent(),at()},{once:!0})},1e3)};setTimeout(()=>{t&&document.body.contains(t)&&(console.log("Preloader safety timeout triggered"),t.classList.add("hiding"),setTimeout(()=>{t.remove(),document.body.classList.remove("preloading"),w.updatePageContent(),at()},400))},5e3),setTimeout(()=>{requestAnimationFrame(a)},300)}function at(){const t=document.getElementById("language-switcher"),e=document.getElementById("language-dropdown");if(!t||!e)return;const s=t.cloneNode(!0);t.parentNode.replaceChild(s,t);const n=w.getCurrentLocale(),a=s.querySelector("span");if(a){const r={en:"English",ru:"Русский","pt-br":"Português"};a.textContent=r[n]||"English"}s.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),e.classList.contains("hidden")?(e.classList.remove("hidden"),s.classList.add("bg-white/10")):(e.classList.add("hidden"),s.classList.remove("bg-white/10"))}),e.querySelectorAll("button").forEach(r=>{r.addEventListener("click",u=>{u.preventDefault(),u.stopPropagation();const c=r.dataset.lang;if(w.setLocale(c),e.classList.add("hidden"),s.classList.remove("bg-white/10"),a){const f={en:"English",ru:"Русский","pt-br":"Português"};a.textContent=f[c]||"English"}M()})}),document.addEventListener("click",r=>{s.contains(r.target)||(e.classList.add("hidden"),s.classList.remove("bg-white/10"))});const i=()=>{const r=t.querySelector("span"),u={en:"English",ru:"Русский","pt-br":"Português"};r.textContent=u[w.currentLocale]||"English"};document.addEventListener("languageChanged",i)}document.addEventListener("DOMContentLoaded",()=>{try{Nt()}catch(o){console.error("Error initializing preloader:",o);const i=document.getElementById("preloader");i&&i.remove(),document.body.classList.remove("preloading"),w.updatePageContent(),at()}te();const t=document.getElementById("accept-cookies"),e=document.getElementById("reject-cookies"),s=document.getElementById("cookie-consent");t&&t.addEventListener("click",()=>{n(),st("cookie-consent","accepted",365)}),e&&e.addEventListener("click",()=>{n(),st("cookie-consent","rejected",365)});function n(){s.classList.add("translate-y-full"),s.addEventListener("transitionend",()=>{s.style.display="none"},{once:!0})}St(),setTimeout(()=>{Ct()},300),se(),ne(),ae(),rt(),zt(),initializeMobileFilters();let a;window.addEventListener("resize",()=>{clearTimeout(a),a=setTimeout(()=>{rt()},100)})});function St(){try{console.log("Initializing sorting..."),document.querySelectorAll(".sort-option").forEach(e=>{e.addEventListener("click",s=>{s.preventDefault(),s.stopPropagation();const n=e.getAttribute("data-sort");if(console.log("Sort option clicked:",n),n){ot(n);const a=document.getElementById("mobile-filters");a&&!a.classList.contains("hidden")&&(a.classList.add("hidden"),document.body.style.overflow="auto"),M()}})});const t=localStorage.getItem("currentSort")||"hot";console.log("Initial sort type:",t),ot(t),console.log("Sorting initialized with type:",t)}catch(t){console.error("Error initializing sorting:",t)}}let v=[],N="",B=null,I=1;const O={mobile:4,tablet:6,desktop:9,wide:15},F=document.getElementById("about-modal"),_t=document.getElementById("about-btn"),Et=document.getElementById("close-about"),_=document.getElementById("suggest-modal"),jt=document.getElementById("suggest-btn"),Ot=document.getElementById("close-suggest"),X=24*60*60*1e3;document.getElementById("mobile-filters-btn");document.getElementById("mobile-filters-modal");document.getElementById("close-mobile-filters");document.getElementById("mobile-filters-content");document.getElementById("reset-filters");document.getElementById("apply-filters");const Z=document.getElementById("active-filters-count"),Ut={tablet:768};function rt(){var n;const t=document.getElementById("filters-sidebar"),e=(n=document.getElementById("mobile-filters-btn"))==null?void 0:n.parentElement;if(!t||!e)return;const s=window.innerWidth<Ut.tablet;t.classList.toggle("hidden",s),e.classList.toggle("hidden",!s)}function zt(){const t=document.getElementById("mobile-filters-btn"),e=document.getElementById("mobile-filters-modal"),s=e==null?void 0:e.querySelector(".flex.flex-col.bg-\\[\\#111\\]"),n=e==null?void 0:e.querySelector(".bg-\\[\\#0A0A0A\\]");if(!t||!e||!s||!n)return;function a(){e.classList.remove("hidden"),e.offsetHeight,n.classList.add("opacity-100"),s.classList.remove("translate-y-full"),document.body.classList.add("overflow-hidden")}function o(){n.classList.remove("opacity-100"),s.classList.add("translate-y-full"),setTimeout(()=>{e.classList.add("hidden"),document.body.classList.remove("overflow-hidden")},300)}t.addEventListener("click",c=>{c.preventDefault(),a()});const i=document.getElementById("close-mobile-filters");i&&i.addEventListener("click",c=>{c.preventDefault(),o()}),e.addEventListener("click",c=>{c.target===e&&o()}),document.addEventListener("keydown",c=>{c.key==="Escape"&&!e.classList.contains("hidden")&&o()});const r=document.getElementById("reset-filters");r&&r.addEventListener("click",c=>{c.preventDefault(),Jt(),Lt()});const u=document.getElementById("apply-filters");u&&u.addEventListener("click",c=>{c.preventDefault(),o(),M()})}function Lt(){const t=Gt();Z&&(t>0?(Z.textContent=`${t} active`,Z.classList.remove("hidden")):Z.classList.add("hidden"))}function Gt(){let t=0;return N&&t++,B&&t++,localStorage.getItem("currentSort")&&t++,t}function Jt(){N="",B=null,localStorage.removeItem("currentSort"),document.querySelectorAll(".status-filter-btn div").forEach(t=>{t.classList.remove("bg-black/40")}),document.querySelectorAll(".games-filter-btn div").forEach(t=>{t.classList.remove("border-emerald-500/20","bg-black/40"),t.classList.add("border-white/5","bg-black/20")}),document.querySelectorAll(".sort-option").forEach(t=>{t.classList.remove("bg-white/10","text-white"),t.classList.add("text-white/70")}),M()}async function Ct(){try{v=(await(await fetch("data/resources.json")).json()).sources;const s="hydra_ratings_cache",n=5*60*1e3,a=()=>{try{const r=localStorage.getItem(s);if(!r)return null;const{timestamp:u,data:c}=JSON.parse(r);if(Date.now()-u<n)return c}catch(r){console.error("Error reading from cache:",r)}return null},o=r=>{try{localStorage.setItem(s,JSON.stringify({timestamp:Date.now(),data:r}))}catch(u){console.error("Error saving to cache:",u)}},i=a();if(i){console.log("Using cached ratings data"),v.forEach(r=>{const u=i[r.title]||{avg:0,total:0};r.rating={avg:parseFloat(u.avg)||0,total:parseInt(u.total)||0}});return}console.log("Fetching fresh ratings data...");try{const r=[...new Set(v.map(p=>p.title))];console.log(`Fetching ratings for ${r.length} sources`);const u=`https://libraryratingsdb.zxcsixx.workers.dev/api/ratings?batch=true&sources=${r.map(encodeURIComponent).join(",")}`;console.log("Batch API URL:",u);const c=await fetch(u);if(console.log("Batch response status:",c.status),!c.ok)throw new Error(`HTTP error! status: ${c.status}`);const f=await c.json();console.log("Received batch ratings data"),o(f),v.forEach(p=>{const S=f[p.title]||{avg:0,total:0};p.rating={avg:parseFloat(S.avg)||0,total:parseInt(S.total)||0}}),console.log("All ratings loaded via batch endpoint")}catch(r){console.error("Error in batch ratings fetch:",r),v.forEach(u=>{u.rating={avg:0,total:0}})}St(),M(),Vt()}catch(t){console.error("Error loading sources:",t);const e=document.getElementById("sources-container");e&&(e.innerHTML=`
    ...
    `;const c=(R=r.querySelector("[data-rating-stars-container]"))==null?void 0:R.parentElement;c&&(c.addEventListener("click",k=>{k.stopPropagation(),Ht(t)}),$t(r,t.rating));const f=r.querySelector(".install-btn"),p=r.querySelector(".copy-btn");return p&&p.addEventListener("click",async()=>{const k=async()=>{await yt(t.url,"copy")&&(navigator.clipboard.writeText(t.url),p.innerHTML='<i class="fas fa-check text-[10px]"></i> '+w.t("sourceCard.copied"),setTimeout(()=>{p.innerHTML='<i class="fas fa-copy text-[10px]"></i> '+w.t("sourceCard.copy")},2e3))},T=t.title==="CPGRepacks";a||T?(T?ht:pt)($=>{$&&k()}):k()}),f&&f.addEventListener("click",async()=>{const k=async()=>{bt(f,"loading");const E=await yt(t.url,"install");if(bt(f,E?"success":"rate-limited"),E){const $=encodeURIComponent(t.url);window.location.href=`hydralauncher://install-source?urls=${$}`}},T=t.title==="CPGRepacks";t.status.includes("Use At Your Own Risk")||T?(T?ht:pt)($=>{$&&k()}):k()}),mt(t.url,t.stats),r.dataset.copies=o.copies||0,r.dataset.installs=o.installs||0,r.dataset.activity=i,r}let Q="wide";function kt(){const t=window.innerWidth;return t>=1536?"wide":t>=1024?"desktop":t>=640?"tablet":"mobile"}let vt;window.addEventListener("resize",()=>{clearTimeout(vt),vt=setTimeout(()=>{rt();const t=kt();if(Q!==t){const s=ut(),n=tt(),a=Math.ceil(s.length/n);if(Q==="wide"&&t==="desktop"){const o=(I-1)*O.desktop;I=Math.floor(o/O.desktop)+1}Q=t,I=Math.min(Math.max(1,I),a),M(s),n<tt()&&window.scrollTo({top:0,behavior:"smooth"})}},100)});function xt(t,e){return console.log(`Sorting sources by: ${e}`),[...t].sort((s,n)=>{var a,o,i,r,u,c,f,p,S,L,x,C;try{switch(e){case"hot":const R=((a=s.stats)==null?void 0:a.recentActivity)||0;return(((o=n.stats)==null?void 0:o.recentActivity)||0)-R;case"new":return new Date(n.addedDate||0)-new Date(s.addedDate||0);case"most-copies":const T=((i=s.stats)==null?void 0:i.copies)||0;return(((r=n.stats)==null?void 0:r.copies)||0)-T;case"most-installs":const $=((u=s.stats)==null?void 0:u.installs)||0;return(((c=n.stats)==null?void 0:c.installs)||0)-$;case"top-rated":console.log(`Comparing ratings: ${s.title} (${((f=s.rating)==null?void 0:f.avg)||0}) vs ${n.title} (${((p=n.rating)==null?void 0:p.avg)||0})`),console.log(`Full rating data for ${s.title}:`,s.rating),console.log(`Full rating data for ${n.title}:`,n.rating);const q=((S=s.rating)==null?void 0:S.avg)||0,H=((L=n.rating)==null?void 0:L.avg)||0;if(q===H){const W=((x=s.rating)==null?void 0:x.total)||0,U=((C=n.rating)==null?void 0:C.total)||0;return console.log(`Ratings equal (${q}), comparing counts: ${W} vs ${U}`),U-W}return console.log(`Sorting by rating: ${H} - ${q} = ${H-q}`),H-q;case"name-asc":return(s.title||"").localeCompare(n.title||"");case"name-desc":return(n.title||"").localeCompare(s.title||"");default:return 0}}catch(R){return console.error("Error in sort function:",R),console.error("Sort type:",e),console.error("Source A:",s),console.error("Source B:",n),0}})}function ot(t){var e;try{console.log("Updating active sort UI for:",t);const s=document.querySelector(".mobile-sort-button span");if(s){const a=((e=document.querySelector(`.sort-option[data-sort="${t}"]`))==null?void 0:e.textContent)||"Sort";s.textContent=a}document.querySelectorAll(".sort-option").forEach(a=>{const o=a.getAttribute("data-sort")===t;a.classList.toggle("bg-white/10",o),a.classList.toggle("text-white",o),a.classList.toggle("text-white/70",!o);const i=a.querySelector("i.fa-check");i&&(i.style.opacity=o?"1":"0")});const n=document.getElementById("sort-dropdown");n&&!n.classList.contains("hidden")&&n.classList.add("hidden"),localStorage.setItem("currentSort",t)}catch(s){console.error("Error updating sort UI:",s)}}function M(t=null){const e=document.getElementById("sources-container");if(e){e.innerHTML="";try{let s=t||ut();console.log(`Displaying ${s.length} filtered sources`);const n=localStorage.getItem("currentSort")||"hot";if(console.log("Current sort type:",n),n){console.log("Before sorting - first few sources:",s.slice(0,3).map(c=>{var f,p;return{title:c.title,rating:c.rating,hasRating:!!((f=c.rating)!=null&&f.avg||(p=c.rating)!=null&&p.total)}}));try{s=xt(s,n),console.log("After sorting - first few sources:",s.slice(0,3).map(c=>{var f,p;return{title:c.title,rating:c.rating,sortKey:n==="top-rated"?`${((f=c.rating)==null?void 0:f.avg)||0} (${((p=c.rating)==null?void 0:p.total)||0} ratings)`:c[n]||"N/A"}})),ot(n)}catch(c){console.error("Error during sorting:",c),s=xt(s,"hot")}}Q=kt();const a=tt(),o=Math.ceil(s.length/a);I=Math.min(Math.max(1,I),o);const i=(I-1)*a,r=Math.min(i+a,s.length);s.slice(i,r).forEach(c=>{const f=Wt(c);e.appendChild(f)}),Yt(o),w.updatePageContent()}catch(s){console.error("Error in displaySources:",s),e.innerHTML=`
            <div class="col-span-full text-center py-10 px-4">
                <div class="text-red-400 text-4xl mb-4">
                    <i class="fas fa-exclamation-triangle"></i>
                </div>
        ...
        `}}}function tt(){const t=window.innerWidth;return t>=2560?O.wide:t>=1024?O.desktop:t>=640?O.tablet:O.mobile}function Yt(t){const e=document.getElementById("pagination");if(!e)return;let s="";s+=`
    ...
    `,e.innerHTML=s}window.changePage=function(t){if(t<1)return;const e=ut(),s=Math.ceil(e.length/tt());t>s||(I=t,M(e),window.scrollTo({top:0,behavior:"smooth"}))};document.querySelectorAll(".status-filter-btn").forEach(t=>{t.addEventListener("click",()=>{const e=t.dataset.status;document.querySelectorAll(".status-filter-btn").forEach(s=>{s.querySelector("div").classList.remove("bg-black/40")}),N===e?N="":(N=e,t.querySelector("div").classList.add("bg-black/40")),I=1,M(),It(),dispatchFiltersChanged()})});document.querySelectorAll(".games-filter-btn").forEach(t=>{t.addEventListener("click",()=>{const e=parseInt(t.dataset.min),s=parseInt(t.dataset.max);if(document.querySelectorAll(".games-filter-btn div").forEach(n=>{n.classList.remove("border-emerald-500/20","bg-black/40"),n.classList.add("border-white/5","bg-black/20")}),B&&B.min===e&&B.max===s)B=null;else{B={min:e,max:s};const n=t.querySelector("div");n.classList.remove("border-white/5","bg-black/20"),n.classList.add("border-emerald-500/20","bg-black/40")}I=1,M(),It(),Lt(),dispatchFiltersChanged()})});function It(){const t={};v.forEach(e=>{e.status.forEach(s=>{t[s]=(t[s]||0)+1})}),document.querySelectorAll(".status-filter-btn").forEach(e=>{const s=e.dataset.status,n=e.querySelector(".text-white\\/40");n&&(n.textContent=t[s]||0)}),document.querySelectorAll(".games-filter-btn").forEach(e=>{const s=parseInt(e.dataset.min),n=parseInt(e.dataset.max),a=v.filter(r=>{const u=parseInt(r.gamesCount);return u>=s&&u<=n}).length,o=e.querySelector(".text-white\\/40");o&&(o.textContent=a);const i=e.querySelector(".bg-emerald-500\\/50");if(i){const r=Math.max(...Array.from(document.querySelectorAll(".games-filter-btn")).map(c=>{const f=parseInt(c.dataset.min),p=parseInt(c.dataset.max);return v.filter(S=>{const L=parseInt(S.gamesCount);return L>=f&&L<=p}).length})),u=r>0?a/r*100:0;i.style.width=`${u}%`}})}let J="";document.getElementById("search").addEventListener("input",t=>{J=t.target.value.toLowerCase(),I=1,M()});function ut(){return v.filter(t=>{const e=!J||t.title.toLowerCase().includes(J)||t.description.toLowerCase().includes(J)||t.url.toLowerCase().includes(J),s=!N||t.status.includes(N),n=parseInt(t.gamesCount),a=!B||n>=B.min&&n<=B.max;return e&&s&&a})}const Tt=document.getElementById("sort-dropdown-btn"),et=document.getElementById("sort-dropdown"),Kt=document.getElementById("current-sort");Tt.addEventListener("click",()=>{et.classList.toggle("hidden")});document.addEventListener("click",t=>{!Tt.contains(t.target)&&!et.contains(t.target)&&et.classList.add("hidden")});document.querySelectorAll(".sort-option").forEach(t=>{t.addEventListener("click",()=>{const e=t.dataset.sort;switch(Kt.textContent=t.textContent.trim(),et.classList.add("hidden"),e){case"high-to-low":sortGamesFilters(!1);break;case"low-to-high":sortGamesFilters(!0);break;case"most-sources":Zt();break;case"most-popular":Qt();break}})});function Zt(){const t=document.querySelector(".games-filter-btn").parentNode,e=Array.from(t.querySelectorAll(".games-filter-btn"));e.sort((s,n)=>{const a=parseInt(s.querySelector(".text-white\\/40").textContent);return parseInt(n.querySelector(".text-white\\/40").textContent)-a}),e.forEach(s=>s.remove()),e.forEach(s=>t.appendChild(s))}function Qt(){const t=JSON.parse(localStorage.getItem("sourceStats")||"{}"),e=document.querySelector(".games-filter-btn").parentNode,s=Array.from(e.querySelectorAll(".games-filter-btn"));s.sort((n,a)=>{var p,S;const o=((p=v.find(L=>L.gamesCount===n.dataset.min))==null?void 0:p.url)||"",i=((S=v.find(L=>L.gamesCount===a.dataset.min))==null?void 0:S.url)||"",r=t[o]||{installs:0,copies:0},u=t[i]||{installs:0,copies:0},c=r.installs+r.copies;return u.installs+u.copies-c}),s.forEach(n=>n.remove()),s.forEach(n=>e.appendChild(n))}function Xt(t){const e=new Date(t),n=Math.abs(new Date-e),a=Math.floor(n/(1e3*60*60*24)),o=w.t.bind(w);if(a===0)return o("common.date.today");if(a===1)return o("common.date.yesterday");if(a<30)return o("common.date.daysAgo",{days:a});{const i={year:"numeric",month:"short",day:"numeric"};return e.toLocaleDateString(w.getCurrentLocale(),i)}}document.addEventListener("DOMContentLoaded",()=>{Ct(),sortGamesFilters(!1)});_t.addEventListener("click",()=>{F.classList.remove("hidden"),document.body.style.overflow="hidden"});Et.addEventListener("click",()=>{F.classList.add("hidden"),document.body.style.overflow=""});F.addEventListener("click",t=>{t.target===F&&Et.click()});document.addEventListener("keydown",t=>{t.key==="Escape"&&!F.classList.contains("hidden")&&(F.classList.add("hidden"),document.body.style.overflow="")});jt.addEventListener("click",()=>{_.classList.remove("hidden"),document.body.style.overflow="hidden"});Ot.addEventListener("click",()=>{_.classList.add("hidden"),document.body.style.overflow=""});_.addEventListener("click",t=>{t.target===_&&(_.classList.add("hidden"),document.body.style.overflow="")});document.addEventListener("keydown",t=>{t.key==="Escape"&&(F.classList.contains("hidden")||(F.classList.add("hidden"),document.body.style.overflow=""),_.classList.contains("hidden")||(_.classList.add("hidden"),document.body.style.overflow=""))});function st(t,e,s){const n=new Date;n.setTime(n.getTime()+s*24*60*60*1e3),document.cookie=`${t}=${e};expires=${n.toUTCString()};path=/`}function At(t){const e=t+"=",s=document.cookie.split(";");for(let n=0;n<s.length;n++){let a=s[n];for(;a.charAt(0)===" ";)a=a.substring(1,a.length);if(a.indexOf(e)===0)return a.substring(e.length,a.length)}return null}async function yt(t,e){try{const s=t.replace(/[^a-zA-Z0-9]/g,"_"),n=`${e}_${s}`;if(At(n))return!0;let a={installs:0,copies:0,activity:[],recentActivity:0};try{const i=new Promise((x,C)=>setTimeout(()=>C(new Error("Firebase request timed out")),3e3)),r=lt(ct,`sources/${s}/stats`),u=dt(r),f=(await Promise.race([u,i])).val()||{installs:0,copies:0,activity:[]},p=Date.now(),L=(Array.isArray(f.activity)?f.activity:[]).filter(x=>p-x<X);L.push(p),a={installs:parseInt(f.installs||0)+(e==="install"?1:0),copies:parseInt(f.copies||0)+(e==="copy"?1:0),activity:L,recentActivity:L.length,lastUpdated:p},await Promise.race([wt(r,a),new Promise((x,C)=>setTimeout(()=>C(new Error("Update timed out")),3e3))]);try{const x=localStorage.getItem("sourceStats");if(x){const C=JSON.parse(x);C[s]||(C[s]={}),C[s].stats=a,localStorage.setItem("sourceStats",JSON.stringify(C)),localStorage.setItem("statsLastUpdated",p.toString())}}catch{}}catch{console.log("Firebase update failed, continuing with local data");const r=v.find(u=>u.url===t);if(r&&r.stats){a={installs:parseInt(r.stats.installs||0)+(e==="install"?1:0),copies:parseInt(r.stats.copies||0)+(e==="copy"?1:0),activity:[],recentActivity:1,lastUpdated:Date.now()};try{const u=localStorage.getItem("sourceStats");if(u){const c=JSON.parse(u);c[s]||(c[s]={}),c[s].stats=a,localStorage.setItem("sourceStats",JSON.stringify(c)),localStorage.setItem("statsLastUpdated",Date.now().toString())}}catch{}}}st(n,"true",e==="install"?.003472222:347222e-9),mt(t,a);const o=v.findIndex(i=>i.url===t);return o!==-1&&(v[o].stats=a),!0}catch{return!1}}function mt(t,e=null){const s=document.querySelectorAll(".source-card"),n=Array.from(s).find(a=>a.dataset.url===t);if(n){const a=n.querySelector(".source-stats");if(a){const o=parseInt((e==null?void 0:e.installs)||0),i=parseInt((e==null?void 0:e.copies)||0),r=parseInt((e==null?void 0:e.recentActivity)||0);a.innerHTML=`
                <span class="flex items-center gap-1.5">
            ...
            `}n.dataset.copies=(e==null?void 0:e.copies)||0,n.dataset.installs=(e==null?void 0:e.installs)||0,n.dataset.activity=(e==null?void 0:e.recentActivity)||0}}function bt(t,e){const s=t.innerHTML;switch(e){case"loading":t.disabled=!0,t.innerHTML=`
            ...
            `,setTimeout(()=>{t.disabled=!1,t.innerHTML=s},2e3);break;default:t.disabled=!1,t.innerHTML=s}}function te(){const t=document.getElementById("cookie-consent");At("cookie-consent")?t.style.display="none":(t.style.display="block",setTimeout(()=>{t.classList.remove("translate-y-full")},500))}document.getElementById("accept-cookies").addEventListener("click",()=>{const t=document.getElementById("cookie-consent");t.classList.add("translate-y-full"),t.addEventListener("transitionend",()=>{t.style.display="none"},{once:!0}),st("cookie-consent","accepted",365)});async function ee(t){try{const e=lt(ct,`sources/${t}/stats`),n=(await dt(e)).val();if(n&&n.activity){const a=n.activity.filter(o=>Date.now()-o<X);a.length!==n.activity.length&&await wt(e,{activity:a})}}catch{}}function se(){const t=document.getElementById("suggest-modal"),e=document.getElementById("suggest-btn"),s=document.getElementById("close-suggest");e&&e.addEventListener("click",()=>{t.classList.remove("hidden")}),s&&s.addEventListener("click",()=>{t.classList.add("hidden")}),t&&t.addEventListener("click",n=>{n.target===t&&t.classList.add("hidden")})}function ne(){const t=document.getElementById("search");if(t){const e=()=>{const n=window.innerWidth<640?"header.searchMobile":"header.search";t.placeholder=w.t(n)};e(),window.addEventListener("resize",e),document.addEventListener("languageChanged",e)}}function ae(){setInterval(()=>{v.forEach(t=>{const e=t.url.replace(/[^a-zA-Z0-9]/g,"_");ee(e)})},60*60*1e3)}async function re(t){const e=v.find(s=>s.title===t);if(e&&e.rating){const s={avg:parseFloat(e.rating.avg||0),total:parseInt(e.rating.total||0)};return it(t,s),!0}return oe()}async function oe(){if(!v||v.length===0)return!1;const t="hydra_ratings_cache",e=5*60*1e3,s=()=>{try{const i=localStorage.getItem(t);if(!i)return null;const{timestamp:r,data:u}=JSON.parse(i);if(Date.now()-r<e)return u}catch(i){console.error("Error reading from cache:",i)}return null},n=i=>{try{localStorage.setItem(t,JSON.stringify({timestamp:Date.now(),data:i}))}catch(r){console.error("Error saving to cache:",r)}},a=s();if(a)return console.log("Using cached ratings data"),v.forEach(i=>{const r=a[i.title]||{avg:0,total:0};i.rating={avg:parseFloat(r.avg)||0,total:parseInt(r.total)||0},it(i.title,i.rating)}),!0;console.log("Fetching fresh ratings data...");const o=[...new Set(v.map(i=>i.title))];if(o.length===0)return!1;try{const i="https://libraryratingsdb.zxcsixx.workers.dev/api/ratings/batch";console.log("Making batch API request to:",i);const r=await fetch(i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sources:o,fields:["avg","total"]})});if(console.log("Batch response status:",r.status),!r.ok)return console.error("Failed to fetch batch ratings:",r.status,r.statusText),!1;const u=await r.json();return console.log("Received batch ratings data for",Object.keys(u).length,"sources"),n(u),v.forEach(c=>{const f=u[c.title]||{avg:0,total:0};c.rating={avg:parseFloat(f.avg)||0,total:parseInt(f.total)||0},it(c.title,c.rating)}),!0}catch(i){return console.error("Error in updateAllSourceRatings:",i),!1}}function it(t,e){document.querySelectorAll(".source-card").forEach(n=>{n.dataset.name===t&&$t(n,e)})}function $t(t,e){console.log("Updating card rating display for:",t.dataset.name,"with data:",JSON.stringify(e,null,2));const s=t.querySelector("[data-rating-stars-active]"),n=t.querySelector("[data-rating-avg]"),a=t.querySelector("[data-rating-total]"),o=t.querySelector("[data-rating-comment]");if(console.log("Found elements:",{starsActive:!!s,avgEl:!!n,totalEl:!!a,commentEl:!!o}),s&&n&&a)if(e&&(typeof e.avg=="number"||e.avg===0)){const i=e.avg/5*100,r=Math.round(i/10)*10;console.log(`Setting rating: avg=${e.avg}, total=${e.total}, starPercentage=${i}%, rounded=${r}%`),s.style.width=`${r}%`,n.textContent=e.avg>0?e.avg.toFixed(1):"–",a.textContent=e.total>0?`(${e.total})`:"",o&&(o.style.display=e.total>0?"inline-block":"none",console.log(`Comment icon display set to: ${o.style.display}`))}else console.log("No valid rating data, setting default state"),s.style.width="0%",n.textContent="–",a.textContent="",o&&(o.style.display="none");else console.error("Missing required rating elements on card:",{starsActive:!!s,avgEl:!!n,totalEl:!!a})}window.updateSourceCardRating=re;document.addEventListener("languageChanged",()=>{M()});
