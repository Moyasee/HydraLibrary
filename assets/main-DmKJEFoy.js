import{c as Ae,i as y,r as ne,d as re,g as oe,u as ve}from"./index-3DWfQYr_.js";const de="https://libraryratingsdb.zxcsixx.workers.dev/api/ratings";function Te(e){const t=new Date(e),a=Math.floor((new Date-t)/1e3);let n=Math.floor(a/31536e3);return n>=1?n+" year"+(n===1?"":"s")+" ago":(n=Math.floor(a/2592e3),n>=1?n+" month"+(n===1?"":"s")+" ago":(n=Math.floor(a/86400),n>=1?n+" day"+(n===1?"":"s")+" ago":(n=Math.floor(a/3600),n>=1?n+" hour"+(n===1?"":"s")+" ago":(n=Math.floor(a/60),n>=1?n+" minute"+(n===1?"":"s")+" ago":"just now"))))}function F(e){return String(e).replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Me(e){let t=localStorage.getItem("hydra_rating_hash");return t||(t=Math.random().toString(36).slice(2)+Date.now(),localStorage.setItem("hydra_rating_hash",t)),t}function qe(e){document.querySelectorAll(".rating-modal").forEach(d=>d.remove());const t=document.createElement("div");t.className="rating-modal fixed inset-0 z-[9999] flex items-center justify-center p-2 sm:p-4",t.innerHTML=`
  ...
  `,document.body.appendChild(t),document.body.style.overflow="hidden",t.querySelector(".close-rating-modal").onclick=r,t.querySelector(".fixed.inset-0").onclick=d=>{d.target===t.querySelector(".fixed.inset-0")&&r()};async function s(d,l="en"){return new Promise((m,h)=>{const x=JSON.stringify({translate:"rapidapi"}),w=new XMLHttpRequest;w.withCredentials=!0,w.addEventListener("readystatechange",function(){if(this.readyState===this.DONE)try{const b=JSON.parse(this.responseText);if(b&&b.translation)m(b.translation);else throw b&&b.status!==200?new Error(b.business_message||"Translation service error"):new Error("Invalid response from translation service")}catch(b){console.error("Translation error:",b,this.responseText),h(b)}}),w.addEventListener("error",()=>{h(new Error("Network error during translation"))});const T=`https://free-google-translator.p.rapidapi.com/external-api/free-google-translator?from=auto&to=${l}&query=${encodeURIComponent(d)}`;w.open("POST",T),w.setRequestHeader("x-rapidapi-key",""),w.setRequestHeader("x-rapidapi-host","free-google-translator.p.rapidapi.com"),w.setRequestHeader("Content-Type","application/json"),w.send(x)})}t.addEventListener("click",async d=>{const l=d.target.closest(".translate-comment");if(!l||l.disabled)return;d.preventDefault();const m=l.closest('div[class*="group relative"]'),h=m?m.querySelector(".comment-text p"):null;if(!h){console.error("Could not find comment text element");return}let x=l.getAttribute("data-original");if(l.classList.contains("active")){x&&(h.textContent=`"${x}"`,l.innerHTML='<i class="fas fa-language text-xs"></i><span class="hidden sm:inline">Translate</span>',l.classList.remove("active"),l.title="Translate this review"),l.disabled=!1;return}x=h.textContent.trim().replace(/^"/,"").replace(/"$/,""),l.setAttribute("data-original",x);const w=l.innerHTML;l.innerHTML='<i class="fas fa-spinner fa-spin text-xs"></i>',l.title="Translating...",l.disabled=!0;try{const T=await s(x,"en");h.textContent=`"${T}"`,l.innerHTML='<i class="fas fa-undo text-xs"></i><span class="hidden sm:inline">Original</span>',l.classList.add("active"),l.title="Show original text",l.disabled=!1}catch(T){console.error("Translation failed:",T),h.textContent=`"${x}"`,l.innerHTML=w,l.title="Error translating. Click to try again."}});const a=t.querySelector('textarea[name="comment"]'),n=t.querySelector(".char-count");if(a&&n){const d=()=>{n.textContent=a.value.length};a.addEventListener("input",d),d(),t._cleanupCharCounter=()=>{a.removeEventListener("input",d)}}function r(){t._cleanupCharCounter&&t._cleanupCharCounter(),t.remove(),document.body.style.overflow=""}const c=5*60*1e3;let o=1,u="recent",i=[],g=!1,p=0,L=e.title;function S(){const d=`ratings_${encodeURIComponent(L)}`,l=localStorage.getItem(d);if(!l)return null;try{const{data:m,timestamp:h}=JSON.parse(l);if(Date.now()-h<c)return m}catch(m){console.warn("Error reading cache:",m)}return null}function $(d){try{const l=`ratings_${encodeURIComponent(L)}`,m={data:d,timestamp:Date.now()};localStorage.setItem(l,JSON.stringify(m))}catch(l){console.warn("Error saving to cache:",l)}}async function E(d=1,l=!1){if(g)return;const m=t.querySelector("#rating-comments-list");if(!m)return;m.innerHTML='<div class="flex justify-center items-center py-6"><div class="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-emerald-400"></div></div>';const h=l?null:S();if(h){console.log("Using cached data for:",L),M(h,d),Date.now()-p>c&&R(!0);return}await R()}async function R(d=!1){if(g)return;g=!0;const l=t.querySelector("#rating-comments-list");try{const m=`${de}?source=${encodeURIComponent(L)}`;console.log("Fetching ratings from:",m);const h=await fetch(m,{headers:{"Cache-Control":"no-cache",Pragma:"no-cache"}});if(!h.ok)throw new Error(`HTTP error! status: ${h.status}`);const x=await h.json();console.log("Received ratings data:",x),$(x),p=Date.now(),M(x,o)}catch(m){console.error("Error loading comments:",m),!d&&l&&(l.innerHTML=`
          <div class="text-red-400 text-sm py-4 text-center">
            Failed to load reviews. ${m.message}
          </div>`)}finally{g=!1}}function M(d,l){i=Array.isArray(d.comments)?d.comments:[],d.total;const m=parseFloat(d.avg)||0;q(m);const h=m%1===0?m.toFixed(0):m.toFixed(m*10%1===0?1:2),x=t.querySelector("#rating-modal-avg"),w=t.querySelector("#rating-modal-total");x&&(x.textContent=d.avg?`${h} / 5`:"No ratings yet"),w&&(w.textContent=d.total?`(${d.total} ${d.total===1?"review":"reviews"})`:""),A(l)}function A(d=1){var ce;const l=document.getElementById("rating-comments-list");if(!l)return;const m=Array.isArray(i)?[...i]:[],h=((ce=document.getElementById("rating-sort-select"))==null?void 0:ce.value)||"recent",x=[...m].sort((f,P)=>h==="recent"?new Date(P.timestamp||0)-new Date(f.timestamp||0):h==="low"?(f.rating||0)-(P.rating||0)||new Date(P.timestamp||0)-new Date(f.timestamp||0):h==="high"?(P.rating||0)-(f.rating||0)||new Date(P.timestamp||0)-new Date(f.timestamp||0):0),w=10,T=Math.ceil(x.length/w),b=Math.min(Math.max(1,d),T||1),G=x.slice((b-1)*w,b*w);if(l.innerHTML=G.length>0?G.map(f=>`
          ...
          <div class="relative">
            ...
            <div class="flex items-start justify-between mb-2">
              <div class="flex items-center gap-2">
                <div class="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-400 text-sm font-medium">
                  ${f.nickname?F(f.nickname.charAt(0).toUpperCase()):"?"}
                </div>
                <div>
                  <div class="font-medium text-white/90">${f.nickname?F(f.nickname):"Anonymous"}</div>
                  <div class="flex items-center gap-1 mt-0.5">
                    <div class="text-amber-400 text-xs flex">
                      ${"★".repeat(f.rating||0)}${"☆".repeat(5-(f.rating||0))}
                    </div>
                    <span class="text-xs text-white/40">•</span>
                    <span class="text-xs text-white/50" title="${F(new Date(f.timestamp||Date.now()).toLocaleString())}">
                      ${F(Te(f.timestamp||Date.now()))} 
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div class="flex items-center gap-2">
                <button 
                  ...
                  data-comment="${f.message?encodeURIComponent(f.message):""}"
                ...
                <div class="w-2 h-2 rounded-full ${(f.rating||0)>=3?"bg-emerald-500":"bg-amber-500"} shadow-pulse"></div>
                <span class="text-xs ${(f.rating||0)>=3?"text-emerald-400":"text-amber-400"}">
                  ${(f.rating||0)>=3?"Positive":"Needs Improvement"}
                </span>
              </div>
            </div>
            ...
            <div class="comment-container">
              <div class="pl-2 border-l-2 border-emerald-500/30 comment-text">
                <p class="text-sm text-white/80 leading-relaxed whitespace-pre-wrap" data-original="${f.message?F(f.message):""}">
                  "${f.message?F(f.message).replace(/\n/g,"<br>"):"No comment provided"}" 
                </p>
              </div>
            </div>
      ...
      `,T>1){let f='<div class="flex flex-wrap gap-2 justify-center mt-6 pt-4 border-t border-white/5">';b>1&&(f+=`
          ...
          </button>`);const P=5;let J=Math.max(1,b-Math.floor(P/2)),ee=Math.min(T,J+P-1);ee-J+1<P&&(J=Math.max(1,ee-P+1));for(let N=J;N<=ee;N++)f+=`
          ...
          <button class="px-3 py-1.5 text-xs rounded-md bg-black/40 text-white/70 border border-white/5 hover:bg-black/60 hover:text-white transition-colors" 
          ...
          </button>`),f+="</div>",l.insertAdjacentHTML("afterend",f),document.querySelectorAll("#rating-comments-list + div [data-page]").forEach(N=>{N.addEventListener("click",$e=>{$e.preventDefault();const V=parseInt(N.dataset.page);!isNaN(V)&&V>=1&&V<=T&&(A(V),window.scrollTo({top:l.offsetTop-20,behavior:"smooth"}))})})}}function C(){const d=t.querySelector("#rating-sort-select");d&&(d.value=u,d.onchange=l=>{u=l.target.value,o=1,A(o)})}C(),E();function q(d){const l=t.querySelector("#rating-modal-stars");l.innerHTML="";for(let m=1;m<=5;++m)m<=Math.round(d)?l.innerHTML+='<svg class="inline-block w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path fill-rule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clip-rule="evenodd" /></svg>':l.innerHTML+='<svg class="inline-block w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" /></svg>'}E();const O=t.querySelector("#submit-rating-form"),k=t.querySelector("#rating-form-error");O.onsubmit=async d=>{d.preventDefault(),k.textContent="",k.className="text-sm text-rose-400 mt-2";const l=new FormData(O),m=(l.get("nickname")||"").trim(),h=l.get("rating"),x=(l.get("comment")||"").trim();if(!m){k.textContent="Please enter a nickname.";return}if(!h||isNaN(h)||h<1||h>5){k.textContent="Please select a valid rating between 1 and 5.";return}if(!x){k.textContent="Please enter a message.";return}if(x.split(/\s+/).length<3){k.textContent="Message must be at least 3 words long.";return}const w=Me(),T=`hydra_rating_${e.title}_${w}`;if(localStorage.getItem(T)){k.textContent="You have already submitted a review for this source.";return}try{const b=await fetch(de,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({source:e.title,nickname:m,rating:Number(h),message:x,ipHash:w})});if(!b.ok){const G=await b.text();k.textContent=G||"Failed to submit.";return}localStorage.setItem(T,"1"),O.reset(),k.textContent="Submitted for moderation!",window.updateSourceCardRating&&await window.updateSourceCardRating(e.title),E(o)}catch{k.textContent="Failed to submit."}}}document.body.classList.add("preloading");document.addEventListener("DOMContentLoaded",async()=>{await Ae.initialize()});function Be(){const e=document.getElementById("preloader"),t=e.querySelector(".loading-progress"),s=e.querySelector(".loading-percentage");let a=0;const n=()=>{const r=100-a,c=Math.min(r*.1,Math.max(.2,Math.random()*.8));a=Math.min(100,a+c),t.style.width=`${a}%`;const o=a<100?a.toFixed(1):Math.round(a);s.textContent=`${o}%`,a<30?s.className="loading-percentage text-sm font-medium text-white/70":a<60?s.className="loading-percentage text-sm font-medium text-emerald-400/70":s.className="loading-percentage text-sm font-medium text-emerald-400",a<100?requestAnimationFrame(n):setTimeout(()=>{e.classList.add("hiding"),e.addEventListener("transitionend",()=>{e.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),te()},{once:!0})},1e3)};setTimeout(()=>{e&&document.body.contains(e)&&(console.log("Preloader safety timeout triggered"),e.classList.add("hiding"),setTimeout(()=>{e.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),te()},400))},5e3),setTimeout(()=>{requestAnimationFrame(n)},300)}function te(){const e=document.getElementById("language-switcher"),t=document.getElementById("language-dropdown");if(!e||!t)return;const s=e.cloneNode(!0);e.parentNode.replaceChild(s,e);const a=y.getCurrentLocale(),n=s.querySelector("span");if(n){const o={en:"English",ru:"Русский","pt-br":"Português"};n.textContent=o[a]||"English"}s.addEventListener("click",o=>{o.preventDefault(),o.stopPropagation(),t.classList.contains("hidden")?(t.classList.remove("hidden"),s.classList.add("bg-white/10")):(t.classList.add("hidden"),s.classList.remove("bg-white/10"))}),t.querySelectorAll("button").forEach(o=>{o.addEventListener("click",u=>{u.preventDefault(),u.stopPropagation();const i=o.dataset.lang;if(y.setLocale(i),t.classList.add("hidden"),s.classList.remove("bg-white/10"),n){const g={en:"English",ru:"Русский","pt-br":"Português"};n.textContent=g[i]||"English"}B()})}),document.addEventListener("click",o=>{s.contains(o.target)||(t.classList.add("hidden"),s.classList.remove("bg-white/10"))});const c=()=>{const o=e.querySelector("span"),u={en:"English",ru:"Русский","pt-br":"Português"};o.textContent=u[y.currentLocale]||"English"};document.addEventListener("languageChanged",c)}document.addEventListener("DOMContentLoaded",()=>{try{Be()}catch(r){console.error("Error initializing preloader:",r);const c=document.getElementById("preloader");c&&c.remove(),document.body.classList.remove("preloading"),y.updatePageContent(),te()}Ye();const e=document.getElementById("accept-cookies"),t=document.getElementById("reject-cookies"),s=document.getElementById("cookie-consent");e&&e.addEventListener("click",()=>{a(),X("cookie-consent","accepted",365)}),t&&t.addEventListener("click",()=>{a(),X("cookie-consent","rejected",365)});function a(){s.classList.add("translate-y-full"),s.addEventListener("transitionend",()=>{s.style.display="none"},{once:!0})}xe(),setTimeout(()=>{we()},300),Ke(),Ze(),Qe(),se(),Ne(),initializeMobileFilters();let n;window.addEventListener("resize",()=>{clearTimeout(n),n=setTimeout(()=>{se()},100)})});function xe(){try{console.log("Initializing sorting..."),document.querySelectorAll(".sort-option").forEach(t=>{t.addEventListener("click",s=>{s.preventDefault(),s.stopPropagation();const a=t.getAttribute("data-sort");if(console.log("Sort option clicked:",a),a){ae(a);const n=document.getElementById("mobile-filters");n&&!n.classList.contains("hidden")&&(n.classList.add("hidden"),document.body.style.overflow="auto"),B()}})});const e=localStorage.getItem("currentSort")||"hot";console.log("Initial sort type:",e),ae(e),console.log("Sorting initialized with type:",e)}catch(e){console.error("Error initializing sorting:",e)}}let v=[],j="",D=null,I=1;const U={mobile:4,tablet:6,desktop:9,wide:15},H=document.getElementById("about-modal"),Pe=document.getElementById("about-btn"),be=document.getElementById("close-about"),z=document.getElementById("suggest-modal"),De=document.getElementById("suggest-btn"),Re=document.getElementById("close-suggest"),K=24*60*60*1e3;document.getElementById("mobile-filters-btn");document.getElementById("mobile-filters-modal");document.getElementById("close-mobile-filters");document.getElementById("mobile-filters-content");document.getElementById("reset-filters");document.getElementById("apply-filters");const Y=document.getElementById("active-filters-count"),Fe={tablet:768};function se(){var a;const e=document.getElementById("filters-sidebar"),t=(a=document.getElementById("mobile-filters-btn"))==null?void 0:a.parentElement;if(!e||!t)return;const s=window.innerWidth<Fe.tablet;e.classList.toggle("hidden",s),t.classList.toggle("hidden",!s)}function Ne(){const e=document.getElementById("mobile-filters-btn"),t=document.getElementById("mobile-filters-modal"),s=t==null?void 0:t.querySelector(".flex.flex-col.bg-\\[\\#111\\]"),a=t==null?void 0:t.querySelector(".bg-\\[\\#0A0A0A\\]");if(!e||!t||!s||!a)return;function n(){t.classList.remove("hidden"),t.offsetHeight,a.classList.add("opacity-100"),s.classList.remove("translate-y-full"),document.body.classList.add("overflow-hidden")}function r(){a.classList.remove("opacity-100"),s.classList.add("translate-y-full"),setTimeout(()=>{t.classList.add("hidden"),document.body.classList.remove("overflow-hidden")},300)}e.addEventListener("click",i=>{i.preventDefault(),n()});const c=document.getElementById("close-mobile-filters");c&&c.addEventListener("click",i=>{i.preventDefault(),r()}),t.addEventListener("click",i=>{i.target===t&&r()}),document.addEventListener("keydown",i=>{i.key==="Escape"&&!t.classList.contains("hidden")&&r()});const o=document.getElementById("reset-filters");o&&o.addEventListener("click",i=>{i.preventDefault(),je(),ye()});const u=document.getElementById("apply-filters");u&&u.addEventListener("click",i=>{i.preventDefault(),r(),B()})}function ye(){const e=He();Y&&(e>0?(Y.textContent=`${e} active`,Y.classList.remove("hidden")):Y.classList.add("hidden"))}function He(){let e=0;return j&&e++,D&&e++,localStorage.getItem("currentSort")&&e++,e}function je(){j="",D=null,localStorage.removeItem("currentSort"),document.querySelectorAll(".status-filter-btn div").forEach(e=>{e.classList.remove("bg-black/40")}),document.querySelectorAll(".games-filter-btn div").forEach(e=>{e.classList.remove("border-emerald-500/20","bg-black/40"),e.classList.add("border-white/5","bg-black/20")}),document.querySelectorAll(".sort-option").forEach(e=>{e.classList.remove("bg-white/10","text-white"),e.classList.add("text-white/70")}),B()}async function we(){try{v=(await(await fetch("data/resources.json")).json()).sources,xe(),B(),ze()}catch(e){console.error("Error loading sources:",e);const t=document.getElementById("sources-container");t&&(t.innerHTML=`
            ...
            `)}finally{setTimeout(()=>{const e=document.getElementById("preloader");e&&(e.style.opacity="0",setTimeout(()=>{e.style.display="none",B()},300))},500)}}async function ze(){try{const e=new Promise((r,c)=>setTimeout(()=>c(new Error("Firebase request timed out")),5e3)),t=ne(re,"sources"),s=oe(t),n=(await Promise.race([s,e])).val();if(n){v=v.map(r=>{var p;const c=r.url.replace(/[^a-zA-Z0-9]/g,"_"),o=((p=n[c])==null?void 0:p.stats)||{installs:0,copies:0,activity:[]},u=Array.isArray(o.activity)?o.activity:[],i=Date.now(),g=u.filter(L=>i-L<K).length;return{...r,stats:{...o,recentActivity:g,activity:u}}}),v.forEach(r=>{le(r.url,r.stats)});try{localStorage.setItem("sourceStats",JSON.stringify(n)),localStorage.setItem("statsLastUpdated",Date.now().toString())}catch{}return!0}return!1}catch{try{const t=localStorage.getItem("sourceStats");if(t){const s=JSON.parse(t);return v=v.map(a=>{var i;const n=a.url.replace(/[^a-zA-Z0-9]/g,"_"),r=((i=s[n])==null?void 0:i.stats)||{installs:0,copies:0,activity:[]},c=Array.isArray(r.activity)?r.activity:[],o=Date.now(),u=c.filter(g=>o-g<K).length;return{...a,stats:{...r,recentActivity:u,activity:c}}}),console.log("Using cached stats from localStorage"),!0}}catch{}return v=v.map(t=>({...t,stats:{installs:0,copies:0,recentActivity:0,activity:[]}})),!1}}function ue(e){document.querySelector("main").classList.add("blur-sm","transition-all","duration-200");const t=y.t.bind(y),s=document.createElement("div");s.className="fixed inset-0 flex items-center justify-center z-50 animate-fade-in p-4",s.innerHTML=`
    ...
    `;const i=(R=o.querySelector("[data-rating-stars-container]"))==null?void 0:R.parentElement;i&&(i.addEventListener("click",M=>{M.stopPropagation(),qe(e)}),Ie(o,e.rating));const g=o.querySelector(".install-btn"),p=o.querySelector(".copy-btn");return p&&p.addEventListener("click",async()=>{const M=async()=>{await pe(e.url,"copy")&&(navigator.clipboard.writeText(e.url),p.innerHTML='<i class="fas fa-check text-[10px]"></i> '+y.t("sourceCard.copied"),setTimeout(()=>{p.innerHTML='<i class="fas fa-copy text-[10px]"></i> '+y.t("sourceCard.copy")},2e3))},A=e.title==="CPGRepacks";n||A?(A?me:ue)(q=>{q&&M()}):M()}),g&&g.addEventListener("click",async()=>{const M=async()=>{he(g,"loading");const C=await pe(e.url,"install");if(he(g,C?"success":"rate-limited"),C){const q=encodeURIComponent(e.url);window.location.href=`hydralauncher://install-source?urls=${q}`}},A=e.title==="CPGRepacks";e.status.includes("Use At Your Own Risk")||A?(A?me:ue)(q=>{q&&M()}):M()}),le(e.url,e.stats),o.dataset.copies=r.copies||0,o.dataset.installs=r.installs||0,o.dataset.activity=c,o}let W="wide";function Le(){const e=window.innerWidth;return e>=1536?"wide":e>=1024?"desktop":e>=640?"tablet":"mobile"}let ge;window.addEventListener("resize",()=>{clearTimeout(ge),ge=setTimeout(()=>{se();const e=Le();if(W!==e){const s=ie(),a=Z(),n=Math.ceil(s.length/a);if(W==="wide"&&e==="desktop"){const r=(I-1)*U.desktop;I=Math.floor(r/U.desktop)+1}W=e,I=Math.min(Math.max(1,I),n),B(s),a<Z()&&window.scrollTo({top:0,behavior:"smooth"})}},100)});function fe(e,t){return console.log(`Sorting sources by: ${t}`),[...e].sort((s,a)=>{var n,r,c,o,u,i,g,p,L,S,$,E;try{switch(t){case"hot":const R=((n=s.stats)==null?void 0:n.recentActivity)||0;return(((r=a.stats)==null?void 0:r.recentActivity)||0)-R;case"new":return new Date(a.addedDate||0)-new Date(s.addedDate||0);case"most-copies":const A=((c=s.stats)==null?void 0:c.copies)||0;return(((o=a.stats)==null?void 0:o.copies)||0)-A;case"most-installs":const q=((u=s.stats)==null?void 0:u.installs)||0;return(((i=a.stats)==null?void 0:i.installs)||0)-q;case"top-rated":console.log(`Comparing ratings: ${s.title} (${((g=s.rating)==null?void 0:g.avg)||0}) vs ${a.title} (${((p=a.rating)==null?void 0:p.avg)||0})`),console.log(`Full rating data for ${s.title}:`,s.rating),console.log(`Full rating data for ${a.title}:`,a.rating);const k=((L=s.rating)==null?void 0:L.avg)||0,d=((S=a.rating)==null?void 0:S.avg)||0;if(k===d){const l=(($=s.rating)==null?void 0:$.total)||0,m=((E=a.rating)==null?void 0:E.total)||0;return console.log(`Ratings equal (${k}), comparing counts: ${l} vs ${m}`),m-l}return console.log(`Sorting by rating: ${d} - ${k} = ${d-k}`),d-k;case"name-asc":return(s.title||"").localeCompare(a.title||"");case"name-desc":return(a.title||"").localeCompare(s.title||"");default:return 0}}catch(R){return console.error("Error in sort function:",R),console.error("Sort type:",t),console.error("Source A:",s),console.error("Source B:",a),0}})}function ae(e){var t;try{console.log("Updating active sort UI for:",e);const s=document.querySelector(".mobile-sort-button span");if(s){const n=((t=document.querySelector(`.sort-option[data-sort="${e}"]`))==null?void 0:t.textContent)||"Sort";s.textContent=n}document.querySelectorAll(".sort-option").forEach(n=>{const r=n.getAttribute("data-sort")===e;n.classList.toggle("bg-white/10",r),n.classList.toggle("text-white",r),n.classList.toggle("text-white/70",!r);const c=n.querySelector("i.fa-check");c&&(c.style.opacity=r?"1":"0")});const a=document.getElementById("sort-dropdown");a&&!a.classList.contains("hidden")&&a.classList.add("hidden"),localStorage.setItem("currentSort",e)}catch(s){console.error("Error updating sort UI:",s)}}function B(e=null){const t=document.getElementById("sources-container");if(t){t.innerHTML="";try{let s=e||ie();console.log(`Displaying ${s.length} filtered sources`);const a=localStorage.getItem("currentSort")||"hot";if(console.log("Current sort type:",a),a){console.log("Before sorting - first few sources:",s.slice(0,3).map(i=>{var g,p;return{title:i.title,rating:i.rating,hasRating:!!((g=i.rating)!=null&&g.avg||(p=i.rating)!=null&&p.total)}}));try{s=fe(s,a),console.log("After sorting - first few sources:",s.slice(0,3).map(i=>{var g,p;return{title:i.title,rating:i.rating,sortKey:a==="top-rated"?`${((g=i.rating)==null?void 0:g.avg)||0} (${((p=i.rating)==null?void 0:p.total)||0} ratings)`:i[a]||"N/A"}})),ae(a)}catch(i){console.error("Error during sorting:",i),s=fe(s,"hot")}}W=Le();const n=Z(),r=Math.ceil(s.length/n);I=Math.min(Math.max(1,I),r);const c=(I-1)*n,o=Math.min(c+n,s.length);s.slice(c,o).forEach(i=>{const g=Oe(i);t.appendChild(g)}),Ue(r),y.updatePageContent()}catch(s){console.error("Error in displaySources:",s),t.innerHTML=`
            <div class="col-span-full text-center py-10 px-4">
                <div class="text-red-400 text-4xl mb-4">
                    <i class="fas fa-exclamation-triangle"></i>
                </div>
        ...
        `}}}function Z(){const e=window.innerWidth;return e>=2560?U.wide:e>=1024?U.desktop:e>=640?U.tablet:U.mobile}function Ue(e){const t=document.getElementById("pagination");if(!t)return;let s="";s+=`
    ...
    `,t.innerHTML=s}window.changePage=function(e){if(e<1)return;const t=ie(),s=Math.ceil(t.length/Z());e>s||(I=e,B(t),window.scrollTo({top:0,behavior:"smooth"}))};document.querySelectorAll(".status-filter-btn").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.status;document.querySelectorAll(".status-filter-btn").forEach(s=>{s.querySelector("div").classList.remove("bg-black/40")}),j===t?j="":(j=t,e.querySelector("div").classList.add("bg-black/40")),I=1,B(),Se(),dispatchFiltersChanged()})});document.querySelectorAll(".games-filter-btn").forEach(e=>{e.addEventListener("click",()=>{const t=parseInt(e.dataset.min),s=parseInt(e.dataset.max);if(document.querySelectorAll(".games-filter-btn div").forEach(a=>{a.classList.remove("border-emerald-500/20","bg-black/40"),a.classList.add("border-white/5","bg-black/20")}),D&&D.min===t&&D.max===s)D=null;else{D={min:t,max:s};const a=e.querySelector("div");a.classList.remove("border-white/5","bg-black/20"),a.classList.add("border-emerald-500/20","bg-black/40")}I=1,B(),Se(),ye(),dispatchFiltersChanged()})});function Se(){const e={};v.forEach(t=>{t.status.forEach(s=>{e[s]=(e[s]||0)+1})}),document.querySelectorAll(".status-filter-btn").forEach(t=>{const s=t.dataset.status,a=t.querySelector(".text-white\\/40");a&&(a.textContent=e[s]||0)}),document.querySelectorAll(".games-filter-btn").forEach(t=>{const s=parseInt(t.dataset.min),a=parseInt(t.dataset.max),n=v.filter(o=>{const u=parseInt(o.gamesCount);return u>=s&&u<=a}).length,r=t.querySelector(".text-white\\/40");r&&(r.textContent=n);const c=t.querySelector(".bg-emerald-500\\/50");if(c){const o=Math.max(...Array.from(document.querySelectorAll(".games-filter-btn")).map(i=>{const g=parseInt(i.dataset.min),p=parseInt(i.dataset.max);return v.filter(L=>{const S=parseInt(L.gamesCount);return S>=g&&S<=p}).length})),u=o>0?n/o*100:0;c.style.width=`${u}%`}})}let _="";document.getElementById("search").addEventListener("input",e=>{_=e.target.value.toLowerCase(),I=1,B()});function ie(){return v.filter(e=>{const t=!_||e.title.toLowerCase().includes(_)||e.description.toLowerCase().includes(_)||e.url.toLowerCase().includes(_),s=!j||e.status.includes(j),a=parseInt(e.gamesCount),n=!D||a>=D.min&&a<=D.max;return t&&s&&n})}const Ee=document.getElementById("sort-dropdown-btn"),Q=document.getElementById("sort-dropdown"),_e=document.getElementById("current-sort");Ee.addEventListener("click",()=>{Q.classList.toggle("hidden")});document.addEventListener("click",e=>{!Ee.contains(e.target)&&!Q.contains(e.target)&&Q.classList.add("hidden")});document.querySelectorAll(".sort-option").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.sort;switch(_e.textContent=e.textContent.trim(),Q.classList.add("hidden"),t){case"high-to-low":sortGamesFilters(!1);break;case"low-to-high":sortGamesFilters(!0);break;case"most-sources":Ge();break;case"most-popular":Je();break}})});function Ge(){const e=document.querySelector(".games-filter-btn").parentNode,t=Array.from(e.querySelectorAll(".games-filter-btn"));t.sort((s,a)=>{const n=parseInt(s.querySelector(".text-white\\/40").textContent);return parseInt(a.querySelector(".text-white\\/40").textContent)-n}),t.forEach(s=>s.remove()),t.forEach(s=>e.appendChild(s))}function Je(){const e=JSON.parse(localStorage.getItem("sourceStats")||"{}"),t=document.querySelector(".games-filter-btn").parentNode,s=Array.from(t.querySelectorAll(".games-filter-btn"));s.sort((a,n)=>{var p,L;const r=((p=v.find(S=>S.gamesCount===a.dataset.min))==null?void 0:p.url)||"",c=((L=v.find(S=>S.gamesCount===n.dataset.min))==null?void 0:L.url)||"",o=e[r]||{installs:0,copies:0},u=e[c]||{installs:0,copies:0},i=o.installs+o.copies;return u.installs+u.copies-i}),s.forEach(a=>a.remove()),s.forEach(a=>t.appendChild(a))}function Ve(e){const t=new Date(e),a=Math.abs(new Date-t),n=Math.floor(a/(1e3*60*60*24)),r=y.t.bind(y);if(n===0)return r("common.date.today");if(n===1)return r("common.date.yesterday");if(n<30)return r("common.date.daysAgo",{days:n});{const c={year:"numeric",month:"short",day:"numeric"};return t.toLocaleDateString(y.getCurrentLocale(),c)}}document.addEventListener("DOMContentLoaded",()=>{we(),sortGamesFilters(!1)});Pe.addEventListener("click",()=>{H.classList.remove("hidden"),document.body.style.overflow="hidden"});be.addEventListener("click",()=>{H.classList.add("hidden"),document.body.style.overflow=""});H.addEventListener("click",e=>{e.target===H&&be.click()});document.addEventListener("keydown",e=>{e.key==="Escape"&&!H.classList.contains("hidden")&&(H.classList.add("hidden"),document.body.style.overflow="")});De.addEventListener("click",()=>{z.classList.remove("hidden"),document.body.style.overflow="hidden"});Re.addEventListener("click",()=>{z.classList.add("hidden"),document.body.style.overflow=""});z.addEventListener("click",e=>{e.target===z&&(z.classList.add("hidden"),document.body.style.overflow="")});document.addEventListener("keydown",e=>{e.key==="Escape"&&(H.classList.contains("hidden")||(H.classList.add("hidden"),document.body.style.overflow=""),z.classList.contains("hidden")||(z.classList.add("hidden"),document.body.style.overflow=""))});function X(e,t,s){const a=new Date;a.setTime(a.getTime()+s*24*60*60*1e3),document.cookie=`${e}=${t};expires=${a.toUTCString()};path=/`}function ke(e){const t=e+"=",s=document.cookie.split(";");for(let a=0;a<s.length;a++){let n=s[a];for(;n.charAt(0)===" ";)n=n.substring(1,n.length);if(n.indexOf(t)===0)return n.substring(t.length,n.length)}return null}async function pe(e,t){try{const s=e.replace(/[^a-zA-Z0-9]/g,"_"),a=`${t}_${s}`;if(ke(a))return!0;let n={installs:0,copies:0,activity:[],recentActivity:0};try{const c=new Promise(($,E)=>setTimeout(()=>E(new Error("Firebase request timed out")),3e3)),o=ne(re,`sources/${s}/stats`),u=oe(o),g=(await Promise.race([u,c])).val()||{installs:0,copies:0,activity:[]},p=Date.now(),S=(Array.isArray(g.activity)?g.activity:[]).filter($=>p-$<K);S.push(p),n={installs:parseInt(g.installs||0)+(t==="install"?1:0),copies:parseInt(g.copies||0)+(t==="copy"?1:0),activity:S,recentActivity:S.length,lastUpdated:p},await Promise.race([ve(o,n),new Promise(($,E)=>setTimeout(()=>E(new Error("Update timed out")),3e3))]);try{const $=localStorage.getItem("sourceStats");if($){const E=JSON.parse($);E[s]||(E[s]={}),E[s].stats=n,localStorage.setItem("sourceStats",JSON.stringify(E)),localStorage.setItem("statsLastUpdated",p.toString())}}catch{}}catch{console.log("Firebase update failed, continuing with local data");const o=v.find(u=>u.url===e);if(o&&o.stats){n={installs:parseInt(o.stats.installs||0)+(t==="install"?1:0),copies:parseInt(o.stats.copies||0)+(t==="copy"?1:0),activity:[],recentActivity:1,lastUpdated:Date.now()};try{const u=localStorage.getItem("sourceStats");if(u){const i=JSON.parse(u);i[s]||(i[s]={}),i[s].stats=n,localStorage.setItem("sourceStats",JSON.stringify(i)),localStorage.setItem("statsLastUpdated",Date.now().toString())}}catch{}}}X(a,"true",t==="install"?.003472222:347222e-9),le(e,n);const r=v.findIndex(c=>c.url===e);return r!==-1&&(v[r].stats=n),!0}catch{return!1}}function le(e,t=null){const s=document.querySelectorAll(".source-card"),a=Array.from(s).find(n=>n.dataset.url===e);if(a){const n=a.querySelector(".source-stats");if(n){const r=parseInt((t==null?void 0:t.installs)||0),c=parseInt((t==null?void 0:t.copies)||0),o=parseInt((t==null?void 0:t.recentActivity)||0);n.innerHTML=`
                <span class="flex items-center gap-1.5">
