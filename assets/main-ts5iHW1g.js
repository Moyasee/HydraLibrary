import{c as Tt,i as w,r as rt,d as ot,g as it,u as vt}from"./index-C1xm5zC9.js";const dt="https://libraryratingsdb.zxcsixx.workers.dev/api/ratings",At=5*60*1e3,Y={data:{},lastUpdated:{},get:function(t){const e=this.data[t],a=this.lastUpdated[t];return e&&a&&Date.now()-a<At?Promise.resolve(e):null},set:function(t,e){return this.data[t]=e,this.lastUpdated[t]=Date.now(),e},batchGet:function(t){const e={cached:{},uncached:[]};return t.forEach(a=>{const s=this.get(a);s?e.cached[a]=s:e.uncached.push(a)}),e},batchSet:function(t){Object.entries(t).forEach(([e,a])=>{this.set(e,a)})}};function Mt(t){const e=new Date(t),s=Math.floor((new Date-e)/1e3);let n=Math.floor(s/31536e3);return n>=1?n+" year"+(n===1?"":"s")+" ago":(n=Math.floor(s/2592e3),n>=1?n+" month"+(n===1?"":"s")+" ago":(n=Math.floor(s/86400),n>=1?n+" day"+(n===1?"":"s")+" ago":(n=Math.floor(s/3600),n>=1?n+" hour"+(n===1?"":"s")+" ago":(n=Math.floor(s/60),n>=1?n+" minute"+(n===1?"":"s")+" ago":"just now"))))}function V(t){return String(t).replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function qt(t){let e=localStorage.getItem("hydra_rating_hash");return e||(e=Math.random().toString(36).slice(2)+Date.now(),localStorage.setItem("hydra_rating_hash",e)),e}function Pt(t){document.querySelectorAll(".rating-modal").forEach(u=>u.remove());const e=document.createElement("div");e.className="rating-modal fixed inset-0 z-[9999] flex items-center justify-center p-2 sm:p-4",e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.body.style.overflow="hidden",e.querySelector(".close-rating-modal").onclick=r,e.querySelector(".fixed.inset-0").onclick=u=>{u.target===e.querySelector(".fixed.inset-0")&&r()};async function a(u,d="en"){return new Promise((f,p)=>{const x=JSON.stringify({translate:"rapidapi"}),g=new XMLHttpRequest;g.withCredentials=!0,g.addEventListener("readystatechange",function(){if(this.readyState===this.DONE)try{const y=JSON.parse(this.responseText);if(y&&y.translation)f(y.translation);else throw y&&y.status!==200?new Error(y.business_message||"Translation service error"):new Error("Invalid response from translation service")}catch(y){console.error("Translation error:",y,this.responseText),p(y)}}),g.addEventListener("error",()=>{p(new Error("Network error during translation"))});const L=`https://free-google-translator.p.rapidapi.com/external-api/free-google-translator?from=auto&to=${d}&query=${encodeURIComponent(u)}`;g.open("POST",L),g.setRequestHeader("x-rapidapi-key",""),g.setRequestHeader("x-rapidapi-host","free-google-translator.p.rapidapi.com"),g.setRequestHeader("Content-Type","application/json"),g.send(x)})}async function s(u){const d=u.target.closest(".translate-comment");if(!d||d.disabled)return;u.preventDefault(),u.stopPropagation();const f=d.closest(".border-b"),p=f?f.querySelector(".comment-text p"):null;if(!p){console.error("Could not find comment text element");return}let x=d.getAttribute("data-original");if(d.classList.contains("active")){x&&(p.textContent=x,d.innerHTML='<i class="fas fa-language text-xs"></i><span class="hidden sm:inline">Translate</span>',d.classList.remove("active"),d.title="Translate this review"),d.disabled=!1;return}x=p.textContent.trim(),d.setAttribute("data-original",x),d.innerHTML,d.innerHTML='<i class="fas fa-spinner fa-spin text-xs"></i>',d.title="Translating...",d.disabled=!0;try{const g=await a(x,"en");p.textContent=g,d.innerHTML='<i class="fas fa-undo text-xs"></i><span class="hidden sm:inline">Original</span>',d.classList.add("active"),d.title="Show original text"}catch(g){console.error("Translation failed:",g),p.textContent=x,d.innerHTML='<i class="fas fa-exclamation-triangle text-xs"></i><span class="hidden sm:inline">Error</span>',d.title="Error translating. Click to try again."}finally{d.disabled=!1}}const n=e.querySelector("#rating-comments-list");n&&n.addEventListener("click",s);const i=e.querySelector('textarea[name="comment"]'),l=e.querySelector(".char-count");if(i&&l){const u=()=>{l.textContent=i.value.length};i.addEventListener("input",u),u(),e._cleanupCharCounter=()=>{i.removeEventListener("input",u)}}function r(){e._cleanupCharCounter&&e._cleanupCharCounter(),e.remove(),document.body.style.overflow=""}const c=5*60*1e3;let o=1,m="recent",h=0,b=!1,S=0,E=t.title;function C(){const u=`ratings_${encodeURIComponent(E)}`,d=localStorage.getItem(u);if(!d)return null;try{const{data:f,timestamp:p}=JSON.parse(d);if(Date.now()-p<c)return f}catch(f){console.warn("Error reading cache:",f)}return null}function H(u){try{const d=`ratings_${encodeURIComponent(E)}`,f={data:u,timestamp:Date.now()};localStorage.setItem(d,JSON.stringify(f))}catch(d){console.warn("Error saving to cache:",d)}}async function $(u=1,d=!1){if(b)return;const f=e.querySelector("#rating-comments-list");if(!f)return;f.innerHTML='<div class="flex justify-center items-center py-6"><div class="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-emerald-400"></div></div>';const p=d?null:C();if(p){console.log("Using cached data for:",E),A(p,u),Date.now()-S>c&&T(!0);return}await T()}function A(u,d=1){const f=e.querySelector("#rating-comments-list");if(f){if(f.innerHTML="",u.avg!==void 0){const p=parseFloat(u.avg).toFixed(1),x=parseInt(u.total)||0,g=e.querySelector("#rating-modal-stars"),L=e.querySelector("#rating-modal-avg"),y=e.querySelector("#rating-modal-total");if(g){g.innerHTML="";const D=Math.round(parseFloat(u.avg));g.innerHTML="★".repeat(D)+"☆".repeat(5-D)}L&&(L.textContent=`${p} out of 5`),y&&(y.textContent=`${x} ${x===1?"review":"reviews"}`)}if(!u.comments||u.comments.length===0){f.innerHTML=`
        <div class="text-center py-6 text-gray-400">
          No reviews yet. Be the first to leave a review!
        </div>
      `;return}H(u),u.pagination&&(h=u.pagination.total||0,k()),u.comments.forEach(p=>{if(!p)return;const x=document.createElement("div");x.className="border-b border-gray-700 py-4";const g=p.timestamp?new Date(p.timestamp).getTime():Date.now();x.innerHTML=`
        <div class="flex items-start justify-between mb-2">
          <div class="flex items-center">
            <div class="text-yellow-400 text-sm">
              ${"★".repeat(Math.round(p.rating||0))}${"☆".repeat(5-Math.round(p.rating||0))}
            </div>
            <span class="ml-2 text-sm text-gray-400">${(p.rating||0).toFixed(1)}</span>
          </div>
          <span class="text-xs text-gray-500">${Mt(g)}</span>
        </div>
        <div class="text-sm text-gray-300 mb-2 comment-text">
          <p>${V(p.message||p.comment||"")}</p>
        </div>
      ...
      `,f.appendChild(x)})}}function k(){const u=e.querySelector(".pagination");if(!u)return;const d=Math.ceil(h/10);let f=`
      <div class="flex justify-between items-center text-sm">
        ...
        <button 
          class="px-3 py-1 rounded ${o>=d?"text-gray-500 cursor-not-allowed":"text-emerald-400 hover:bg-gray-700"}"
          ${o>=d?"disabled":""}
          onclick="document.querySelector('.rating-modal')._vm.loadPage(${o+1})"
        >
          Next
    ...
    `;u.innerHTML=f}window.loadPage=u=>{var f;if(!q)return;const d=Math.ceil((((f=q.comments)==null?void 0:f.length)||0)/M);u<1||u>d||(o=u,R())},e._vm={loadPage:window.loadPage};async function T(u=!1){if(b)return;b=!0;const d=e.querySelector("#rating-comments-list");!u&&d&&(d.innerHTML='<div class="text-center py-4 text-gray-400">Loading comments...</div>');const f=`comments_${E}`,p=5*60*1e3,x=Y.get(f);if(x&&Date.now()-x.timestamp<p){q=x,R(),b=!1,Date.now()-x.timestamp>p/2&&(console.log("Cache is stale, refreshing in background..."),T(!0).catch(console.error));return}try{if(window._batchRatingRequests||(window._batchRatingRequests={}),window._batchRatingRequests[f]){console.log("Batch request already in progress, waiting..."),await window._batchRatingRequests[f];const g=Y.get(f);if(g){q=g,R(),b=!1;return}}window._batchRatingRequests[f]=(async()=>{var g,L;try{const y=await fetch(`${dt}?source=${encodeURIComponent(E)}&all=true`);if(!y.ok)throw new Error(`Failed to fetch comments: ${y.statusText}`);const D=await y.json(),_={...D,comments:D.comments||[],pagination:{total:((g=D.comments)==null?void 0:g.length)||0,page:1,limit:((L=D.comments)==null?void 0:L.length)||0,totalPages:1},timestamp:Date.now()};return Y.set(f,_),S=Date.now(),_}finally{delete window._batchRatingRequests[f]}})(),q=await window._batchRatingRequests[f],R()}catch(g){console.error("Error fetching comments:",g),x?(q=x,R()):d&&(d.innerHTML=`
          <div class="text-center py-4 text-red-400">
            Failed to load comments. Please try again later.
            ${g.message?`<div class="text-xs mt-1">${g.message}</div>`:""}
          </div>
        `)}finally{b=!1}}let q=null;const M=10;function R(){if(!q)return;let u=[...q.comments||[]];switch(m){case"recent":u.sort((g,L)=>{const y=g.timestamp?new Date(g.timestamp).getTime():0;return(L.timestamp?new Date(L.timestamp).getTime():0)-y});break;case"high":u.sort((g,L)=>(L.rating||0)-(g.rating||0));break;case"low":u.sort((g,L)=>(g.rating||0)-(L.rating||0));break}const d=u.length,f=Math.ceil(d/M)||1;o=Math.min(Math.max(1,o),f);const p=(o-1)*M,x=u.slice(p,p+M);A({...q,comments:x,pagination:{total:d,page:o,limit:M,totalPages:f}},o)}function J(){const u=e.querySelector("#rating-sort-select");u&&(u.value=m,u.addEventListener("change",d=>{m=d.target.value,o=1,R()}))}J(),$(),$();const j=e.querySelector("#submit-rating-form"),B=e.querySelector("#rating-form-error");j.onsubmit=async u=>{u.preventDefault(),B.textContent="",B.className="text-sm text-rose-400 mt-2";const d=new FormData(j),f=(d.get("nickname")||"").trim(),p=d.get("rating"),x=(d.get("comment")||"").trim();if(!f){B.textContent="Please enter a nickname.";return}if(!p||isNaN(p)||p<1||p>5){B.textContent="Please select a valid rating between 1 and 5.";return}if(!x){B.textContent="Please enter a message.";return}if(x.split(/\s+/).length<3){B.textContent="Message must be at least 3 words long.";return}const g=qt(),L=`hydra_rating_${t.title}_${g}`;if(localStorage.getItem(L)){B.textContent="You have already submitted a review for this source.";return}const y=j.querySelector('button[type="submit"]'),D=y.innerHTML;y.disabled=!0,y.innerHTML="Submitting...";try{const _=await fetch(dt,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({source:t.title,nickname:f,rating:Number(p),message:x,ipHash:g})});if(!_.ok){const $t=await _.json().catch(()=>({}));throw new Error($t.message||"Failed to submit. Please try again.")}j.reset(),B.className="text-sm text-emerald-400 mt-2",B.textContent="Thank you! Your review has been submitted for moderation.",localStorage.setItem(L,"1");const It=`comments_${t.title}_${o}_${m}`;Y.set(It,null),window.updateSourceCardRating&&window.updateSourceCardRating(t.title),setTimeout(()=>{$(1)},1e3)}catch(_){console.error("Submission error:",_),B.textContent=_.message||"Failed to submit. Please try again.";return}finally{y.disabled=!1,y.innerHTML=D}}}document.body.classList.add("preloading");document.addEventListener("DOMContentLoaded",async()=>{await Tt.initialize()});function Rt(){const t=document.getElementById("preloader"),e=t.querySelector(".loading-progress"),a=t.querySelector(".loading-percentage");let s=0;const n=()=>{const i=100-s,l=Math.min(i*.1,Math.max(.2,Math.random()*.8));s=Math.min(100,s+l),e.style.width=`${s}%`;const r=s<100?s.toFixed(1):Math.round(s);a.textContent=`${r}%`,s<30?a.className="loading-percentage text-sm font-medium text-white/70":s<60?a.className="loading-percentage text-sm font-medium text-emerald-400/70":a.className="loading-percentage text-sm font-medium text-emerald-400",s<100?requestAnimationFrame(n):setTimeout(()=>{t.classList.add("hiding"),t.addEventListener("transitionend",()=>{t.remove(),document.body.classList.remove("preloading"),w.updatePageContent(),at()},{once:!0})},1e3)};setTimeout(()=>{t&&document.body.contains(t)&&(console.log("Preloader safety timeout triggered"),t.classList.add("hiding"),setTimeout(()=>{t.remove(),document.body.classList.remove("preloading"),w.updatePageContent(),at()},400))},5e3),setTimeout(()=>{requestAnimationFrame(n)},300)}function at(){const t=document.getElementById("language-switcher"),e=document.getElementById("language-dropdown");if(!t||!e)return;const a=t.cloneNode(!0);t.parentNode.replaceChild(a,t);const s=w.getCurrentLocale(),n=a.querySelector("span");if(n){const r={en:"English",ru:"Русский","pt-br":"Português"};n.textContent=r[s]||"English"}a.addEventListener("click",r=>{r.preventDefault(),r.stopPropagation(),e.classList.contains("hidden")?(e.classList.remove("hidden"),a.classList.add("bg-white/10")):(e.classList.add("hidden"),a.classList.remove("bg-white/10"))}),e.querySelectorAll("button").forEach(r=>{r.addEventListener("click",c=>{c.preventDefault(),c.stopPropagation();const o=r.dataset.lang;if(w.setLocale(o),e.classList.add("hidden"),a.classList.remove("bg-white/10"),n){const m={en:"English",ru:"Русский","pt-br":"Português"};n.textContent=m[o]||"English"}P()})}),document.addEventListener("click",r=>{a.contains(r.target)||(e.classList.add("hidden"),a.classList.remove("bg-white/10"))});const l=()=>{const r=t.querySelector("span"),c={en:"English",ru:"Русский","pt-br":"Português"};r.textContent=c[w.currentLocale]||"English"};document.addEventListener("languageChanged",l)}document.addEventListener("DOMContentLoaded",()=>{try{Rt()}catch(i){console.error("Error initializing preloader:",i);const l=document.getElementById("preloader");l&&l.remove(),document.body.classList.remove("preloading"),w.updatePageContent(),at()}Wt();const t=document.getElementById("accept-cookies"),e=document.getElementById("reject-cookies"),a=document.getElementById("cookie-consent");t&&t.addEventListener("click",()=>{s(),et("cookie-consent","accepted",365)}),e&&e.addEventListener("click",()=>{s(),et("cookie-consent","rejected",365)});function s(){a.classList.add("translate-y-full"),a.addEventListener("transitionend",()=>{a.style.display="none"},{once:!0})}xt(),setTimeout(()=>{wt()},300),Zt(),Qt(),Xt(),st(),_t(),initializeMobileFilters();let n;window.addEventListener("resize",()=>{clearTimeout(n),n=setTimeout(()=>{st()},100)})});function xt(){try{console.log("Initializing sorting..."),document.querySelectorAll(".sort-option").forEach(e=>{e.addEventListener("click",a=>{a.preventDefault(),a.stopPropagation();const s=e.getAttribute("data-sort");if(console.log("Sort option clicked:",s),s){nt(s);const n=document.getElementById("mobile-filters");n&&!n.classList.contains("hidden")&&(n.classList.add("hidden"),document.body.style.overflow="auto"),P()}})});const t=localStorage.getItem("currentSort")||"hot";console.log("Initial sort type:",t),nt(t),console.log("Sorting initialized with type:",t)}catch(t){console.error("Error initializing sorting:",t)}}let v=[],U="",F=null,I=1;const z={mobile:4,tablet:6,desktop:9,wide:15},N=document.getElementById("about-modal"),Bt=document.getElementById("about-btn"),yt=document.getElementById("close-about"),O=document.getElementById("suggest-modal"),Dt=document.getElementById("suggest-btn"),Ft=document.getElementById("close-suggest"),Q=24*60*60*1e3;document.getElementById("mobile-filters-btn");document.getElementById("mobile-filters-modal");document.getElementById("close-mobile-filters");document.getElementById("mobile-filters-content");document.getElementById("reset-filters");document.getElementById("apply-filters");const W=document.getElementById("active-filters-count"),Ht={tablet:768};function st(){var s;const t=document.getElementById("filters-sidebar"),e=(s=document.getElementById("mobile-filters-btn"))==null?void 0:s.parentElement;if(!t||!e)return;const a=window.innerWidth<Ht.tablet;t.classList.toggle("hidden",a),e.classList.toggle("hidden",!a)}function _t(){const t=document.getElementById("mobile-filters-btn"),e=document.getElementById("mobile-filters-modal"),a=e==null?void 0:e.querySelector(".flex.flex-col.bg-\\[\\#111\\]"),s=e==null?void 0:e.querySelector(".bg-\\[\\#0A0A0A\\]");if(!t||!e||!a||!s)return;function n(){e.classList.remove("hidden"),e.offsetHeight,s.classList.add("opacity-100"),a.classList.remove("translate-y-full"),document.body.classList.add("overflow-hidden")}function i(){s.classList.remove("opacity-100"),a.classList.add("translate-y-full"),setTimeout(()=>{e.classList.add("hidden"),document.body.classList.remove("overflow-hidden")},300)}t.addEventListener("click",o=>{o.preventDefault(),n()});const l=document.getElementById("close-mobile-filters");l&&l.addEventListener("click",o=>{o.preventDefault(),i()}),e.addEventListener("click",o=>{o.target===e&&i()}),document.addEventListener("keydown",o=>{o.key==="Escape"&&!e.classList.contains("hidden")&&i()});const r=document.getElementById("reset-filters");r&&r.addEventListener("click",o=>{o.preventDefault(),jt(),bt()});const c=document.getElementById("apply-filters");c&&c.addEventListener("click",o=>{o.preventDefault(),i(),P()})}function bt(){const t=Nt();W&&(t>0?(W.textContent=`${t} active`,W.classList.remove("hidden")):W.classList.add("hidden"))}function Nt(){let t=0;return U&&t++,F&&t++,localStorage.getItem("currentSort")&&t++,t}function jt(){U="",F=null,localStorage.removeItem("currentSort"),document.querySelectorAll(".status-filter-btn div").forEach(t=>{t.classList.remove("bg-black/40")}),document.querySelectorAll(".games-filter-btn div").forEach(t=>{t.classList.remove("border-emerald-500/20","bg-black/40"),t.classList.add("border-white/5","bg-black/20")}),document.querySelectorAll(".sort-option").forEach(t=>{t.classList.remove("bg-white/10","text-white"),t.classList.add("text-white/70")}),P()}async function wt(){try{v=(await(await fetch("data/resources.json")).json()).sources;const a="hydra_ratings_cache",s=5*60*1e3,n=()=>{try{const r=localStorage.getItem(a);if(!r)return null;const{timestamp:c,data:o}=JSON.parse(r);if(Date.now()-c<s)return o}catch(r){console.error("Error reading from cache:",r)}return null},i=r=>{try{localStorage.setItem(a,JSON.stringify({timestamp:Date.now(),data:r}))}catch(c){console.error("Error saving to cache:",c)}},l=n();if(l){console.log("Using cached ratings data"),v.forEach(r=>{const c=l[r.title]||{avg:0,total:0};r.rating={avg:parseFloat(c.avg)||0,total:parseInt(c.total)||0}});return}console.log("Fetching fresh ratings data...");try{const r=[...new Set(v.map(h=>h.title))];console.log(`Fetching ratings for ${r.length} sources`);const c=`https://libraryratingsdb.zxcsixx.workers.dev/api/ratings?batch=true&sources=${r.map(encodeURIComponent).join(",")}`;console.log("Batch API URL:",c);const o=await fetch(c);if(console.log("Batch response status:",o.status),!o.ok)throw new Error(`HTTP error! status: ${o.status}`);const m=await o.json();console.log("Received batch ratings data"),i(m),v.forEach(h=>{const b=m[h.title]||{avg:0,total:0};h.rating={avg:parseFloat(b.avg)||0,total:parseInt(b.total)||0}}),console.log("All ratings loaded via batch endpoint")}catch(r){console.error("Error in batch ratings fetch:",r),v.forEach(c=>{c.rating={avg:0,total:0}})}xt(),P(),Ut()}catch(t){console.error("Error loading sources:",t);const e=document.getElementById("sources-container");e&&(e.innerHTML=`
    ...
    `;const o=(H=r.querySelector("[data-rating-stars-container]"))==null?void 0:H.parentElement;o&&(o.addEventListener("click",$=>{$.stopPropagation(),Pt(t)}),kt(r,t.rating));const m=r.querySelector(".install-btn"),h=r.querySelector(".copy-btn");return h&&h.addEventListener("click",async()=>{const $=async()=>{await pt(t.url,"copy")&&(navigator.clipboard.writeText(t.url),h.innerHTML='<i class="fas fa-check text-[10px]"></i> '+w.t("sourceCard.copied"),setTimeout(()=>{h.innerHTML='<i class="fas fa-copy text-[10px]"></i> '+w.t("sourceCard.copy")},2e3))},A=t.title==="CPGRepacks";n||A?(A?mt:ut)(T=>{T&&$()}):$()}),m&&m.addEventListener("click",async()=>{const $=async()=>{ht(m,"loading");const k=await pt(t.url,"install");if(ht(m,k?"success":"rate-limited"),k){const T=encodeURIComponent(t.url);window.location.href=`hydralauncher://install-source?urls=${T}`}},A=t.title==="CPGRepacks";t.status.includes("Use At Your Own Risk")||A?(A?mt:ut)(T=>{T&&$()}):$()}),ct(t.url,t.stats),r.dataset.copies=i.copies||0,r.dataset.installs=i.installs||0,r.dataset.activity=l,r}let K="wide";function St(){const t=window.innerWidth;return t>=1536?"wide":t>=1024?"desktop":t>=640?"tablet":"mobile"}let gt;window.addEventListener("resize",()=>{clearTimeout(gt),gt=setTimeout(()=>{st();const t=St();if(K!==t){const a=lt(),s=X(),n=Math.ceil(a.length/s);if(K==="wide"&&t==="desktop"){const i=(I-1)*z.desktop;I=Math.floor(i/z.desktop)+1}K=t,I=Math.min(Math.max(1,I),n),P(a),s<X()&&window.scrollTo({top:0,behavior:"smooth"})}},100)});function ft(t,e){return console.log(`Sorting sources by: ${e}`),[...t].sort((a,s)=>{var n,i,l,r,c,o,m,h,b,S,E,C;try{switch(e){case"hot":const H=((n=a.stats)==null?void 0:n.recentActivity)||0;return(((i=s.stats)==null?void 0:i.recentActivity)||0)-H;case"new":return new Date(s.addedDate||0)-new Date(a.addedDate||0);case"most-copies":const A=((l=a.stats)==null?void 0:l.copies)||0;return(((r=s.stats)==null?void 0:r.copies)||0)-A;case"most-installs":const T=((c=a.stats)==null?void 0:c.installs)||0;return(((o=s.stats)==null?void 0:o.installs)||0)-T;case"top-rated":console.log(`Comparing ratings: ${a.title} (${((m=a.rating)==null?void 0:m.avg)||0}) vs ${s.title} (${((h=s.rating)==null?void 0:h.avg)||0})`),console.log(`Full rating data for ${a.title}:`,a.rating),console.log(`Full rating data for ${s.title}:`,s.rating);const M=((b=a.rating)==null?void 0:b.avg)||0,R=((S=s.rating)==null?void 0:S.avg)||0;if(M===R){const J=((E=a.rating)==null?void 0:E.total)||0,j=((C=s.rating)==null?void 0:C.total)||0;return console.log(`Ratings equal (${M}), comparing counts: ${J} vs ${j}`),j-J}return console.log(`Sorting by rating: ${R} - ${M} = ${R-M}`),R-M;case"name-asc":return(a.title||"").localeCompare(s.title||"");case"name-desc":return(s.title||"").localeCompare(a.title||"");default:return 0}}catch(H){return console.error("Error in sort function:",H),console.error("Sort type:",e),console.error("Source A:",a),console.error("Source B:",s),0}})}function nt(t){var e;try{console.log("Updating active sort UI for:",t);const a=document.querySelector(".mobile-sort-button span");if(a){const n=((e=document.querySelector(`.sort-option[data-sort="${t}"]`))==null?void 0:e.textContent)||"Sort";a.textContent=n}document.querySelectorAll(".sort-option").forEach(n=>{const i=n.getAttribute("data-sort")===t;n.classList.toggle("bg-white/10",i),n.classList.toggle("text-white",i),n.classList.toggle("text-white/70",!i);const l=n.querySelector("i.fa-check");l&&(l.style.opacity=i?"1":"0")});const s=document.getElementById("sort-dropdown");s&&!s.classList.contains("hidden")&&s.classList.add("hidden"),localStorage.setItem("currentSort",t)}catch(a){console.error("Error updating sort UI:",a)}}function P(t=null){const e=document.getElementById("sources-container");if(e){e.innerHTML="";try{let a=t||lt();console.log(`Displaying ${a.length} filtered sources`);const s=localStorage.getItem("currentSort")||"hot";if(console.log("Current sort type:",s),s){console.log("Before sorting - first few sources:",a.slice(0,3).map(o=>{var m,h;return{title:o.title,rating:o.rating,hasRating:!!((m=o.rating)!=null&&m.avg||(h=o.rating)!=null&&h.total)}}));try{a=ft(a,s),console.log("After sorting - first few sources:",a.slice(0,3).map(o=>{var m,h;return{title:o.title,rating:o.rating,sortKey:s==="top-rated"?`${((m=o.rating)==null?void 0:m.avg)||0} (${((h=o.rating)==null?void 0:h.total)||0} ratings)`:o[s]||"N/A"}})),nt(s)}catch(o){console.error("Error during sorting:",o),a=ft(a,"hot")}}K=St();const n=X(),i=Math.ceil(a.length/n);I=Math.min(Math.max(1,I),i);const l=(I-1)*n,r=Math.min(l+n,a.length);a.slice(l,r).forEach(o=>{const m=Ot(o);e.appendChild(m)}),zt(i),w.updatePageContent()}catch(a){console.error("Error in displaySources:",a),e.innerHTML=`
            <div class="col-span-full text-center py-10 px-4">
                <div class="text-red-400 text-4xl mb-4">
                    <i class="fas fa-exclamation-triangle"></i>
                </div>
        ...
        `}}}function X(){const t=window.innerWidth;return t>=2560?z.wide:t>=1024?z.desktop:t>=640?z.tablet:z.mobile}function zt(t){const e=document.getElementById("pagination");if(!e)return;let a="";a+=`
    ...
    `,e.innerHTML=a}window.changePage=function(t){if(t<1)return;const e=lt(),a=Math.ceil(e.length/X());t>a||(I=t,P(e),window.scrollTo({top:0,behavior:"smooth"}))};document.querySelectorAll(".status-filter-btn").forEach(t=>{t.addEventListener("click",()=>{const e=t.dataset.status;document.querySelectorAll(".status-filter-btn").forEach(a=>{a.querySelector("div").classList.remove("bg-black/40")}),U===e?U="":(U=e,t.querySelector("div").classList.add("bg-black/40")),I=1,P(),Et(),dispatchFiltersChanged()})});document.querySelectorAll(".games-filter-btn").forEach(t=>{t.addEventListener("click",()=>{const e=parseInt(t.dataset.min),a=parseInt(t.dataset.max);if(document.querySelectorAll(".games-filter-btn div").forEach(s=>{s.classList.remove("border-emerald-500/20","bg-black/40"),s.classList.add("border-white/5","bg-black/20")}),F&&F.min===e&&F.max===a)F=null;else{F={min:e,max:a};const s=t.querySelector("div");s.classList.remove("border-white/5","bg-black/20"),s.classList.add("border-emerald-500/20","bg-black/40")}I=1,P(),Et(),bt(),dispatchFiltersChanged()})});function Et(){const t={};v.forEach(e=>{e.status.forEach(a=>{t[a]=(t[a]||0)+1})}),document.querySelectorAll(".status-filter-btn").forEach(e=>{const a=e.dataset.status,s=e.querySelector(".text-white\\/40");s&&(s.textContent=t[a]||0)}),document.querySelectorAll(".games-filter-btn").forEach(e=>{const a=parseInt(e.dataset.min),s=parseInt(e.dataset.max),n=v.filter(r=>{const c=parseInt(r.gamesCount);return c>=a&&c<=s}).length,i=e.querySelector(".text-white\\/40");i&&(i.textContent=n);const l=e.querySelector(".bg-emerald-500\\/50");if(l){const r=Math.max(...Array.from(document.querySelectorAll(".games-filter-btn")).map(o=>{const m=parseInt(o.dataset.min),h=parseInt(o.dataset.max);return v.filter(b=>{const S=parseInt(b.gamesCount);return S>=m&&S<=h}).length})),c=r>0?n/r*100:0;l.style.width=`${c}%`}})}let G="";document.getElementById("search").addEventListener("input",t=>{G=t.target.value.toLowerCase(),I=1,P()});function lt(){return v.filter(t=>{const e=!G||t.title.toLowerCase().includes(G)||t.description.toLowerCase().includes(G)||t.url.toLowerCase().includes(G),a=!U||t.status.includes(U),s=parseInt(t.gamesCount),n=!F||s>=F.min&&s<=F.max;return e&&a&&n})}const Lt=document.getElementById("sort-dropdown-btn"),tt=document.getElementById("sort-dropdown"),Gt=document.getElementById("current-sort");Lt.addEventListener("click",()=>{tt.classList.toggle("hidden")});document.addEventListener("click",t=>{!Lt.contains(t.target)&&!tt.contains(t.target)&&tt.classList.add("hidden")});document.querySelectorAll(".sort-option").forEach(t=>{t.addEventListener("click",()=>{const e=t.dataset.sort;switch(Gt.textContent=t.textContent.trim(),tt.classList.add("hidden"),e){case"high-to-low":sortGamesFilters(!1);break;case"low-to-high":sortGamesFilters(!0);break;case"most-sources":Jt();break;case"most-popular":Yt();break}})});function Jt(){const t=document.querySelector(".games-filter-btn").parentNode,e=Array.from(t.querySelectorAll(".games-filter-btn"));e.sort((a,s)=>{const n=parseInt(a.querySelector(".text-white\\/40").textContent);return parseInt(s.querySelector(".text-white\\/40").textContent)-n}),e.forEach(a=>a.remove()),e.forEach(a=>t.appendChild(a))}function Yt(){const t=JSON.parse(localStorage.getItem("sourceStats")||"{}"),e=document.querySelector(".games-filter-btn").parentNode,a=Array.from(e.querySelectorAll(".games-filter-btn"));a.sort((s,n)=>{var h,b;const i=((h=v.find(S=>S.gamesCount===s.dataset.min))==null?void 0:h.url)||"",l=((b=v.find(S=>S.gamesCount===n.dataset.min))==null?void 0:b.url)||"",r=t[i]||{installs:0,copies:0},c=t[l]||{installs:0,copies:0},o=r.installs+r.copies;return c.installs+c.copies-o}),a.forEach(s=>s.remove()),a.forEach(s=>e.appendChild(s))}function Vt(t){const e=new Date(t),s=Math.abs(new Date-e),n=Math.floor(s/(1e3*60*60*24)),i=w.t.bind(w);if(n===0)return i("common.date.today");if(n===1)return i("common.date.yesterday");if(n<30)return i("common.date.daysAgo",{days:n});{const l={year:"numeric",month:"short",day:"numeric"};return e.toLocaleDateString(w.getCurrentLocale(),l)}}document.addEventListener("DOMContentLoaded",()=>{wt(),sortGamesFilters(!1)});Bt.addEventListener("click",()=>{N.classList.remove("hidden"),document.body.style.overflow="hidden"});yt.addEventListener("click",()=>{N.classList.add("hidden"),document.body.style.overflow=""});N.addEventListener("click",t=>{t.target===N&&yt.click()});document.addEventListener("keydown",t=>{t.key==="Escape"&&!N.classList.contains("hidden")&&(N.classList.add("hidden"),document.body.style.overflow="")});Dt.addEventListener("click",()=>{O.classList.remove("hidden"),document.body.style.overflow="hidden"});Ft.addEventListener("click",()=>{O.classList.add("hidden"),document.body.style.overflow=""});O.addEventListener("click",t=>{t.target===O&&(O.classList.add("hidden"),document.body.style.overflow="")});document.addEventListener("keydown",t=>{t.key==="Escape"&&(N.classList.contains("hidden")||(N.classList.add("hidden"),document.body.style.overflow=""),O.classList.contains("hidden")||(O.classList.add("hidden"),document.body.style.overflow=""))});function et(t,e,a){const s=new Date;s.setTime(s.getTime()+a*24*60*60*1e3),document.cookie=`${t}=${e};expires=${s.toUTCString()};path=/`}function Ct(t){const e=t+"=",a=document.cookie.split(";");for(let s=0;s<a.length;s++){let n=a[s];for(;n.charAt(0)===" ";)n=n.substring(1,n.length);if(n.indexOf(e)===0)return n.substring(e.length,n.length)}return null}async function pt(t,e){try{const a=t.replace(/[^a-zA-Z0-9]/g,"_"),s=`${e}_${a}`;if(Ct(s))return!0;let n={installs:0,copies:0,activity:[],recentActivity:0};try{const l=new Promise((E,C)=>setTimeout(()=>C(new Error("Firebase request timed out")),3e3)),r=rt(ot,`sources/${a}/stats`),c=it(r),m=(await Promise.race([c,l])).val()||{installs:0,copies:0,activity:[]},h=Date.now(),S=(Array.isArray(m.activity)?m.activity:[]).filter(E=>h-E<Q);S.push(h),n={installs:parseInt(m.installs||0)+(e==="install"?1:0),copies:parseInt(m.copies||0)+(e==="copy"?1:0),activity:S,recentActivity:S.length,lastUpdated:h},await Promise.race([vt(r,n),new Promise((E,C)=>setTimeout(()=>C(new Error("Update timed out")),3e3))]);try{const E=localStorage.getItem("sourceStats");if(E){const C=JSON.parse(E);C[a]||(C[a]={}),C[a].stats=n,localStorage.setItem("sourceStats",JSON.stringify(C)),localStorage.setItem("statsLastUpdated",h.toString())}}catch{}}catch{console.log("Firebase update failed, continuing with local data");const r=v.find(c=>c.url===t);if(r&&r.stats){n={installs:parseInt(r.stats.installs||0)+(e==="install"?1:0),copies:parseInt(r.stats.copies||0)+(e==="copy"?1:0),activity:[],recentActivity:1,lastUpdated:Date.now()};try{const c=localStorage.getItem("sourceStats");if(c){const o=JSON.parse(c);o[a]||(o[a]={}),o[a].stats=n,localStorage.setItem("sourceStats",JSON.stringify(o)),localStorage.setItem("statsLastUpdated",Date.now().toString())}}catch{}}}et(s,"true",e==="install"?.003472222:347222e-9),ct(t,n);const i=v.findIndex(l=>l.url===t);return i!==-1&&(v[i].stats=n),!0}catch{return!1}}function ct(t,e=null){const a=document.querySelectorAll(".source-card"),s=Array.from(a).find(n=>n.dataset.url===t);if(s){const n=s.querySelector(".source-stats");if(n){const i=parseInt((e==null?void 0:e.installs)||0),l=parseInt((e==null?void 0:e.copies)||0),r=parseInt((e==null?void 0:e.recentActivity)||0);n.innerHTML=`
                <span class="flex items-center gap-1.5">
